...
(()=>{
  ...
  async function refreshArenaInfo() {
    ...
    try {
      ...
      if (!scriptElm) throw new Error('grid script.ng');
      const scriptContent = scriptElm.textContent;

      const cellColorsMatch = scriptContent.match(/const cellColors = ({.+?})/s);
      if (!cellColorsMatch) throw new Error('cellColors.ng');
      const cellColorsString = cellColorsMatch[1];
      ...
      if (!capitalMapMatch) throw new Error('capitalMap.ng');
      const capitalMapString = capitalMapMatch[1];
  ...
  async function autoJoin() {
    ...
    function saveCurrentCapital(row, col) {
      if (!(MODE === 'm=hc' || MODE === 'm=l')) return;

      currentCapital = [Number(row), Number(col)];
      sessionStorage.setItem('aat_current_capital', JSON.stringify({
        row: Number(row),
        col: Number(col),
        teamName,
        teamColor
    ...
    function loadCurrentCapital() {
      try {
        const saved = JSON.parse(sessionStorage.getItem('aat_current_capital') || '{}');
        if (
          saved &&
          Number.isInteger(saved.row) &&
          Number.isInteger(saved.col) &&
          (!saved.teamColor || saved.teamColor === teamColor)
        ) {
          currentCapital = [saved.row, saved.col];
        }
      } catch (e) {
        console.error(e);
      }
    ...
    function getCapitalAdjacentCells(row, col, rows, cols) {
    ...
    let nextProgress;
    async function attackRegionSimple () {
      await drawProgressBar();

      if (currentProgress === 0 || currentProgress === 50) {
//追加、これを入れておけば0%や50%での永久ループでのフリーズ回避
      ...
      while(dialog.open) {
        ...
        for (let i = 0; i < regions[cellType].length;) {
          ...
          try {
            ...
            if ((MODE === 'm=hc' || MODE === 'm=l') && text.includes('リーダーになった')) {
            ...
            if (messageType === 'capitalAttack') {
              ...
              message = lastLine;
              cellType = 'onceMore';
//追加、umeRモードのときにはonceMoreがforceFillTypeに移っているのでumeRモードで適用させるため
...
//                }
    ...
    let nextProgressUmeR = null;
    let autoJoinPhase = 'fill';       // fill | defend
    let autoJoinHalf = null;          // first | second
    let cooldownUntil = 0;
    let lastThreatSnapshot = new Set();
    let cooldownReason = '';
    let noTargetLogged = false;
    let forceFillType = '';
    let capitalReflectWaitCount = 0;
    let cooldownStepMs = 20000;

    function regionKey(region) {
      return `${region[0]},${region[1]}`;
    }

    function getBattleHalf(progress) {
      return progress < 50 ? 'first' : 'second';
    }

    function getStopProgress(progress) {
      return progress < 50 ? 49 : 99;
    }

    function isBattleWindow(progress) {
//追加修正、>=50になっててそれだと50%が含まれてしまい50%でも攻撃してしまうため、>50に修正
      return (progress >= 0 && progress < 49) || (progress > 50 && progress < 99);
//追加修正
    }

    function resetAutoJoinStateByProgress(progress) {
      const half = getBattleHalf(progress);
      if (autoJoinHalf !== half) {
        autoJoinHalf = half;
        autoJoinPhase = 'fill';
        cooldownUntil = 0;
        cooldownReason = '';
    ...
    function extendCooldownStep(reason = '') {
      const remain = Math.max(0, cooldownUntil - Date.now());
      const base = remain > 0 ? remain : 0;
      const next = Math.min(base + cooldownStepMs, 60000);
      cooldownUntil = Date.now() + next;
      cooldownReason = reason;
    }
    ...
    function isLikelyEnemyColor(color) {
      const c = normalizeColor(color);
      return !!c
        && c !== normalizeColor(teamColor)
        && c !== 'ffffff'
        && c !== 'ffffff00'
        && c !== '00000000'
        && c !== 'c0c0c0';
    ...
    async function isRegionOwnedByTeam(region) {
      const { cellColors } = await fetchBoardInfo();
      const key = `${region[0]}-${region[1]}`;
      return normalizeColor(cellColors[key]) === normalizeColor(teamColor);
    }

    async function getDefendTargets(regions) {
      const { cellColors } = await fetchBoardInfo();

      const pickEnemyOwned = (cells) => {
      const seen = new Set();
      return (cells || []).filter(region => {
        const key = `${region[0]}-${region[1]}`;
        if (seen.has(key)) return false;
        seen.add(key);
        return isLikelyEnemyColor(cellColors[key]);
      });
    };

     const teamEnemy = pickEnemyOwned(regions.teamAdjacent);
     if (teamEnemy.length > 0) return teamEnemy;

     const capitalEnemy = pickEnemyOwned(regions.capitalAdjacent);
     if (capitalEnemy.length > 0) return capitalEnemy;

     const onceMoreEnemy = pickEnemyOwned(regions.onceMore);
     if (onceMoreEnemy.length > 0) return onceMoreEnemy;

     const nonAdjacentEnemy = pickEnemyOwned(regions.nonAdjacent);
     if (nonAdjacentEnemy.length > 0) return nonAdjacentEnemy;
...
//追加
      ...
      const stopProgress = getStopProgress(currentProgress);
      if (nextProgressUmeR == null || nextProgressUmeR !== stopProgress) {
        nextProgressUmeR = stopProgress;
      }
      if (currentProgress >= nextProgressUmeR) return;

      if (isCooldownActive()) {
        const remain = Math.ceil((cooldownUntil - Date.now()) / 1000);
        if (!noTargetLogged) {
          logMessage(null, `[待機] ${cooldownReason || 'クールダウン中'}`, `→ ${remain}s`);
          noTargetLogged = true;
        }
        return;
      }
...
//追加修正、首都が反映されるまで1.5秒待機
            if (forceFillType === 'ownCapitalAdjacent' && (!regions.ownCapitalAdjacent || regions.ownCapitalAdjacent.length === 0)) {
              if (capitalReflectWaitCount < 2) {
                capitalReflectWaitCount += 1;
                logMessage(null, '[待機] 首都領土の反映待機', `→ 1.5秒後にマップ再取得 (${capitalReflectWaitCount}/2)`);
                await sleep(1500);
                regions = await getRegions();
                continue;
              } else {
                capitalReflectWaitCount = 0;
                forceFillType = 'teamAdjacent';
                logMessage(null, '[移行] 首都周囲未反映のため teamAdjacent へ', '→ 再取得');
                regions = await getRegions();
              }
            }
//追加修正
//追加修正、同士討ちを避けるため
            const orderedTypes = forceFillType
              ...
              : ['ownCapitalAdjacent','teamAdjacent','nonAdjacent','capitalAdjacent','mapEdge'];
//追加修正、下のコメントアウト3行が元のコード
//          const orderedTypes = forceFillType
//            ? [forceFillType, 'ownCapitalAdjacent', 'teamAdjacent', 'nonAdjacent', 'capitalAdjacent', 'mapEdge']
//            : ['ownCapitalAdjacent', 'teamAdjacent', 'nonAdjacent', 'capitalAdjacent', 'mapEdge'];
...
//          }
          ...
          phaseLabel = 'fill';
            ...
            targets = await getDefendTargets(regions);

            const currentThreats = new Set(targets.map(regionKey));
            if (lastThreatSnapshot.size === 0) {
              lastThreatSnapshot = currentThreats;
            } else {
              const newlyThreatened = targets.filter(region => !lastThreatSnapshot.has(regionKey(region)));
              lastThreatSnapshot = currentThreats;
              if (newlyThreatened.length > 0) {
                targets = newlyThreatened;
              }
            }

            if (targets.length === 0) {
              if (!noTargetLogged) {
                logMessage(null, '[待機] 応戦対象なし', `→ ${nextProgressUmeR}%で停止`);
                noTargetLogged = true;
              }
              return;
            }
          ...
          targets = targets.filter(region => !excludeSet.has(regionKey(region)));

          if (targets.length === 0) {
            regions = await getRegions();
            if (autoJoinPhase === 'defend') {
              return;
            }
            continue;
          }

          for (let i = 0; i < targets.length && dialog.open; i++) {
...
//追加
            while (dialog.open) {
              ...
              if (!isBattleWindow(currentProgress) || currentProgress >= nextProgressUmeR) {
                logMessage(region, '[停止] 49% / 99% 到達のため停止', `→ ${nextProgressUmeR}%で停止`);
                return;
              }
              ...
              if (isCooldownActive()) {
                const remain = Math.ceil((cooldownUntil - Date.now()) / 1000);
                if (!noTargetLogged) {
                  logMessage(region, `[待機] ${cooldownReason || 'クールダウン中'}`, `→ ${remain}s`);
                  noTargetLogged = true;
                }
                return;
              }
              ...
              if (equipChangeStat === 'noEquip') {
                excludeSet.add(key);
                noTargetLogged = false;
                logMessage(region, `[除外] 装備不足 (${cellRank})`, `→ ${nextProgressUmeR}%で停止`);
                break;
              }

              const territoryCountBeforeAttack = regions.ownTerritoryCount || 0;

              const [ text, lastLine ] = await challenge(region);
              const messageType = getMessageType(lastLine);

              if (messageType === 'quit') {
                logMessage(region, '[停止] ' + lastLine, '');
                clearInterval(autoJoinIntervalId);
                return;
              }

              if (messageType === 'breaktime') {
                logMessage(region, lastLine, '');
                return;
              }
              ...
              if (messageType === 'equipError') {
                excludeSet.add(key);
                noTargetLogged = false;
                logMessage(region, `[除外] ${lastLine} (${cellRank})`, '→ 次へ');
                regions = await getRegions();
                break;
              }
              ...
              if (messageType === 'guardError') {
                excludeSet.add(key);
                noTargetLogged = false;
                logMessage(region, `[除外] ${lastLine}`, `→ ${nextProgressUmeR}%で停止`);
                regions = await getRegions();
                break;
              }

              if (messageType === 'toofast') {
                startCooldown(3000, 'too fast');
                noTargetLogged = false;
                const remain = Math.ceil((cooldownUntil - Date.now()) / 1000);
                logMessage(region, '[待機] too fast', `→ ${remain}s`);
                return;
              }

              if (messageType === 'retry') {
                noTargetLogged = false;
                logMessage(region, '[待機] retry', '→ 3s');
                await sleep(3000);
                regions = await getRegions();
                break;
              }
              ...
              if (
                lastLine.includes('あなたのチームは動きを使い果たしました') ||
                lastLine.includes('しばらくお待ちください')
              ) {
                extendCooldownStep('行動不能');
                noTargetLogged = false;
                const remain = Math.ceil((cooldownUntil - Date.now()) / 1000);
                logMessage(region, '[待機] 行動不能を検知', `→ ${remain}s`);
                return;
              }

              if (messageType === 'onemoretime') {
                logMessage(region, lastLine, '→ 90s');
                await sleep(90000);
                regions = await getRegions();
                break;
              }
              ...
              if (messageType === 'reset') {
                logMessage(region, lastLine, '→ 再取得');
                regions = await getRegions();
                break;
              }
             if (messageType in regions) {
               noTargetLogged = false;

               if (autoJoinPhase === 'fill') {
                 // 首都を置いた直後は ownCapitalAdjacent 優先を維持
                 if (forceFillType === 'ownCapitalAdjacent' && messageType === 'teamAdjacent') {
                   // 何もしない
                 } else if (messageType === 'teamAdjacent') {
                   forceFillType = 'teamAdjacent';
                 } else if (messageType === 'nonAdjacent') {
                   forceFillType = 'nonAdjacent';
                 } else if (messageType === 'capitalAdjacent') {
                   forceFillType = 'capitalAdjacent';
                 } else if (messageType === 'mapEdge') {
                   forceFillType = 'mapEdge';
                 }
               }

               logMessage(region, `[再取得] ${lastLine}`, '→ 再取得');
               regions = await getRegions();
               break;
             }
              const won = await isRegionOwnedByTeam(region);

              if (won) {
                lastThreatSnapshot.delete(key);
                noTargetLogged = false;

                if (
                  autoJoinPhase === 'fill' &&
//削除、territoryCountBeforeAttack === 0 &&を削除
...
//追加
                ...
                if (forceFillType === 'ownCapitalAdjacent') {
                  capitalReflectWaitCount = 0;
                }

                logMessage(region, `[勝利] ${lastLine}`, phaseLabel === 'fill' ? '埋め成功' : '防衛成功');
                regions = await getRegions();
                break;
              }
              ...
              if (loop >= 255) {
                excludeSet.add(key);
                logMessage(region, `[打止] (${loop}発目) ${lastLine}`, `→ ${nextProgressUmeR}%で停止`);
                break;
              }

              noTargetLogged = false;
              logMessage(region, `(${loop}発目) ${lastLine}`, '→ 1s');
              await sleep(1000);
            }
          }
    ...
    async function attackRegion () {
      if (isMorningTime()) {
        return attackRegionUmeR();
      }
      return attackRegionSimple();
    }
	...
	if (isMorning) {
	  const nonAdjacentBase = cells.filter(([r, c]) => {
	    ...
	    let ownCapitalAdjacentCells = [];

            if (MODE === 'm=hc' || MODE === 'm=l') {
              try {
                // 盤面上の「自軍色の首都」を毎回探す
                const ownCapitalCells = cells.filter(([r, c]) => {
                  const key = `${r}-${c}`;

                  if (!capitalSet.has(key)) return false;
                  if (!cellColors[key]) return false;

                  const color = String(cellColors[key] || '').replace('#', '').toLowerCase();
                  const myColor = String(teamColor || '').replace('#', '').toLowerCase();
                  return color !== '' && color === myColor;
                });

                // 首都はチームで1つ想定。見つかった首都の周囲4マスを候補化
                if (ownCapitalCells.length > 0) {
                  const [capRow, capCol] = ownCapitalCells[0];

                  ownCapitalAdjacentCells = getCapitalAdjacentCells(capRow, capCol, rows, cols)
                    .filter(([r, c]) => {
                      const key = `${r}-${c}`;
                      return !teamColorSet.has(key);
                    });
                }
              } catch (e) {
                console.error(e);
          ...
          regions = {
            ownCapitalAdjacent: shuffle(filteredCells(ownCapitalAdjacentCells)),
        ...
        return {
          ...regions,
          ownTerritoryCount: teamColorSet.size
        };
      } catch (e) {
        console.error(e);
        return {
    ...
    autoJoinIntervalId = setInterval(attackRegion,5000);
  };
