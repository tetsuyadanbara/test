...
//                }
    ...
    let forceFillType = '';
    let capitalCrossStepsRemaining = 0;
...
//追加修正、同士討ちを避けるため
            const orderedTypes = (capitalCrossStepsRemaining > 0)
...
//追加
            while (dialog.open) {
             ...
             if (messageType in regions) {
               ...
               if (autoJoinPhase === 'fill') {
                 if (
                   (capitalCrossStepsRemaining > 0 && messageType === 'teamAdjacent') ||
                   (forceFillType === 'ownCapitalAdjacent' && messageType === 'teamAdjacent')
                 ) {
                   // 首都十字モード中は ownCapitalAdjacent 優先を維持
                 } else if (messageType === 'teamAdjacent') {
              ...
              if (won) {
                ...
                if (
                  autoJoinPhase === 'fill' &&
                  territoryCountBeforeAttack === 0 &&を削除
                  lastLine.includes('リーダーになった')
                ) {
                  saveCurrentCapital(region[0], region[1]);
                  regions = await getRegions();
                  forceFillType = 'ownCapitalAdjacent';
                  capitalCrossStepsRemaining = (regions.ownCapitalAdjacent || []).length;
                  logMessage(region, `[勝利] ${lastLine}`, '首都十字へ');
                  break;
                }
   
                if (capitalCrossStepsRemaining > 0) {
                  capitalCrossStepsRemaining -= 1;
                }
