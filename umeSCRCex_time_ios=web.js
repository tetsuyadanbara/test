...
(()=>{
  const GIST_SYNC_KEYS = [
    'equipPresets',
    'autoEquipItems',
    'autoEquipItemsAutojoin',
    'necklaceCandidateIds',
    'current_equip'
  ];

  const GIST_CONFIG_STORAGE_PREFIX = 'aat_gist_config_v1__';
  const GIST_DEFAULT_FILENAME = 'donguri-equip.json';
  const GIST_DEFAULT_DESCRIPTION = 'donguri equip backup';

  function __aatToStoredString(value) {
    if (value == null) return null;
    return typeof value === 'string' ? value : String(value);
  }

  function __aatCanUseGMValueStorage() {
  ...
  function extractDonguriAccountName(raw) {
    const text = String(raw || '').trim();
    if (!text) return '';

    const idStyle = text.match(/(ハンター|警備員)\s*\[?\s*ID\s*[:：]\s*([a-f0-9]{6,})\s*\]?/i);
    if (idStyle) {
      const role = idStyle[1];
      const id = idStyle[2].toLowerCase();
      return `${role}_${id}`;
    }

    const inlineIdStyle = text.match(/(?:アカウント名|ユーザー名|ニックネーム|名前)\s*[:：]\s*(ハンター|警備員)\s*\[?\s*ID\s*[:：]\s*([a-f0-9]{6,})\s*\]?/i);
    if (inlineIdStyle) {
      const role = inlineIdStyle[1];
      const id = inlineIdStyle[2].toLowerCase();
      return `${role}_${id}`;
    }

    const guardIdOnly = text.match(/警備員\s*ID\s*[:：]\s*([a-f0-9]{6,})/i);
    if (guardIdOnly) {
      return `警備員_${guardIdOnly[1].toLowerCase()}`;
    }

    const hunterIdOnly = text.match(/ハンター\s*ID\s*[:：]\s*([a-f0-9]{6,})/i);
    if (hunterIdOnly) {
      return `ハンター_${hunterIdOnly[1].toLowerCase()}`;
    }

    const roleNearId = text.match(/(ハンター|警備員)[^\n]{0,20}?ID\s*[:：]\s*([a-f0-9]{6,})/i);
    if (roleNearId) {
      return `${roleNearId[1]}_${roleNearId[2].toLowerCase()}`;
    }

    return text;
  }

  function pickAccountNameFromSelectors() {
    const selectors = [
      '[data-account-name]',
      '[data-user-name]',
      '.account-name',
    ...
    const ngWords = new Set([
      'ホーム', 'メニュー', '設定', 'ログアウト', 'ログイン',
      'マイページ', 'プロフィール', 'アイテムバッグ', 'どんぐり基地'
    ]);
    ...
    for (const selector of selectors) {
      ...
      const datasetName =
        elm.dataset?.accountName ||
        elm.dataset?.userName ||
        '';

      const raw = (datasetName || elm.textContent || '').trim();
      if (!raw) continue;
      if (ngWords.has(raw)) continue;

      const detected = extractDonguriAccountName(raw);
      if (detected && !ngWords.has(detected)) return detected;
    }

    return '';
  }

  function pickAccountNameFromText() {
    const bodyText = document.body?.innerText || '';
    const titleText = document.title || '';
    const text = `${bodyText}\n${titleText}`;

    const patterns = [
      /((?:ハンター|警備員)\s*\[?\s*ID\s*[:：]\s*[a-f0-9]{6,}\s*\]?)/i,
      /((?:ハンター|警備員)\s*ID\s*[:：]\s*[a-f0-9]{6,})/i,
      /((?:アカウント名|ユーザー名|ニックネーム|名前)\s*[:：]\s*(?:ハンター|警備員)\s*\[?\s*ID\s*[:：]\s*[a-f0-9]{6,}\s*\]?)/i,
      /((?:アカウント名|ユーザー名|ニックネーム|名前)\s*[:：]\s*[^\n]+)/i,
      /(ようこそ\s*[^\s\n]+\s*さん)/i,
      /([^\s\n]+\s*さん\s*でログイン中)/i,
      /((?:ハンター|警備員)[^\n]{0,20}?ID\s*[:：]\s*[a-f0-9]{6,})/i
    ];
    ...
    for (const re of patterns) {
      ...
      if (m?.[1]) {
        const detected = extractDonguriAccountName(m[1]);
  ...
  function getDetectedDonguriAccountName() {
    return (
      pickAccountNameFromSelectors() ||
      pickAccountNameFromText() ||
      ''
    ).trim();
  }

  function getDetectedGistAccountNameCache() {
    try {
      const s = JSON.parse(localStorage.getItem('aat_settings') || '{}');
      return String(s.detectedGistAccountNameCache || '').trim();
    } catch (e) {
      return '';
    }
  }

  function setDetectedGistAccountNameCache(value) {
    let s;
    try {
      s = JSON.parse(localStorage.getItem('aat_settings') || '{}');
    } catch (e) {
      s = {};
    }
    ...
    if (next) {
      s.detectedGistAccountNameCache = next;
    } else {
      delete s.detectedGistAccountNameCache;
    }

    localStorage.setItem('aat_settings', JSON.stringify(s));
    return next;
  }

  function captureDetectedGistAccountName() {
    const detected = String(getDetectedDonguriAccountName() || '').trim();
    if (!detected) return '';
    setDetectedGistAccountNameCache(detected);
    rememberGistAccountName(detected);
    return detected;
  }

  function getManualGistAccountName() {
    try {
      const s = JSON.parse(localStorage.getItem('aat_settings') || '{}');
      return String(s.gistManualAccountName || '').trim();
    } catch (e) {
      return '';
    }
  }

  function setManualGistAccountName(value) {
    let s;
    try {
      s = JSON.parse(localStorage.getItem('aat_settings') || '{}');
    } catch (e) {
    ...
    const next = String(value || '').trim();
    if (next) {
      s.gistManualAccountName = next;
    } else {
      delete s.gistManualAccountName;
    }
  ...
  function getResolvedGistAccountName() {
    return (
      getDetectedGistAccountNameCache() ||
      getManualGistAccountName() ||
      getDetectedDonguriAccountName() ||
      ''
    ).trim();
  }

  function getCurrentGistAccountKey() {
    const resolved = sanitizeAccountKey(getResolvedGistAccountName());
    return resolved || 'default';
  }

  function getCurrentGistAccountLabel() {
    return getResolvedGistAccountName() || 'default';
  }

  function getManualAccountDisplayText() {
    const autoDetected = getDetectedGistAccountNameCache();
    const manual = getManualGistAccountName();

    if (autoDetected) return `Acct: ${autoDetected} (auto)`;
    if (manual) return `Acct: ${manual}`;
    return 'Acct: 未設定';
  }

  function getGistAccountNameList() {
    try {
      const s = JSON.parse(localStorage.getItem('aat_settings') || '{}');
      const rawList = Array.isArray(s.gistAccountNameList) ? s.gistAccountNameList : [];
      const list = [];
      for (const name of rawList) {
        const v = String(name || '').trim();
        if (v && !list.includes(v)) list.push(v);
      }
      const manual = String(s.gistManualAccountName || '').trim();
  ...
  function saveGistAccountNameList(list) {
    let s;
    try {
      s = JSON.parse(localStorage.getItem('aat_settings') || '{}');
    } catch (e) {
    ...
    for (const name of Array.isArray(list) ? list : []) {
      const v = String(name || '').trim();
      if (v && !normalized.includes(v)) normalized.push(v);
    }

    s.gistAccountNameList = normalized;
    localStorage.setItem('aat_settings', JSON.stringify(s));
    return normalized;
  }

  function rememberGistAccountName(name) {
    const next = String(name || '').trim();
    if (!next) return getGistAccountNameList();

    const list = getGistAccountNameList();
    if (!list.includes(next)) list.push(next);
    return saveGistAccountNameList(list);
  }

  function refreshGistAccountSelect(select) {
    if (!select) return;

    const current = getManualGistAccountName();
    const list = getGistAccountNameList();
  ...
  function refreshManualAccountUi(root = document) {
    const info = root.querySelector('.equip-manual-account-info');
    if (info) info.textContent = getManualAccountDisplayText();
  }

  function getGistConfigStorageKey(accountKey = getCurrentGistAccountKey()) {
    return GIST_CONFIG_STORAGE_PREFIX + sanitizeAccountKey(accountKey || 'default');
  }

  function normalizeGistConfig(input = {}) {
    return {
      token: String(input.token || '').trim(),
      gistId: extractGistId(input.gistId || ''),
      filename: String(input.filename || '').trim() || GIST_DEFAULT_FILENAME,
      description: String(input.description || '').trim() || GIST_DEFAULT_DESCRIPTION,
      accountKey: sanitizeAccountKey(input.accountKey || getCurrentGistAccountKey()) || 'default'
    };
  }

  function readSecureConfigRaw(accountKey = getCurrentGistAccountKey()) {
    const storageKey = getGistConfigStorageKey(accountKey);
    if (__aatCanUseGMValueStorage()) {
      return GM_getValue(storageKey, '');
    }
    return localStorage.getItem(storageKey) || '';
  }

  function writeSecureConfigRaw(value, accountKey = getCurrentGistAccountKey()) {
    const storageKey = getGistConfigStorageKey(accountKey);
    if (__aatCanUseGMValueStorage()) {
  ...
  function getGistConfig(accountKey = getCurrentGistAccountKey()) {
    try {
      return normalizeGistConfig({
        ...JSON.parse(readSecureConfigRaw(accountKey) || '{}'),
        accountKey
      });
    } catch (e) {
      return normalizeGistConfig({ accountKey });
    }
  }

  function saveGistConfig(partial = {}, accountKey = getCurrentGistAccountKey()) {
    const next = normalizeGistConfig({
      ...getGistConfig(accountKey),
      ...partial,
      accountKey
    });
    writeSecureConfigRaw(JSON.stringify(next), accountKey);
    return next;
  }

  function isGistAuthError(error) {
    const msg = String(error?.message || error || '');
    return /\bHTTP\s*(401|403)\b/.test(msg);
  }

  function promptAndUpdateGistToken(accountKey = getCurrentGistAccountKey(), reason = '') {
    const current = getGistConfig(accountKey);
    const message =
      'GitHub token を再入力してください（401/403）' +
      (reason ? `\n理由: ${reason}` : '') +
      `\n対象アカウント: ${current.accountKey || accountKey}`;

    const token = prompt(message, current.token || '');
    if (token === null) return null;

    return saveGistConfig({ token }, accountKey);
  }

  function promptAndSaveGistConfig() {
    let accountName = getResolvedGistAccountName();

    if (!accountName) {
      const manualName = prompt(
        'どんぐりアカウント名を入力してください（手入力固定。例: 零號）',
        ''
      );
      if (manualName === null) return null;

      accountName = String(manualName || '').trim();
      setManualGistAccountName(accountName);
      rememberGistAccountName(accountName);
    } else {
      rememberGistAccountName(accountName);
    }

    const accountKey = sanitizeAccountKey(accountName) || 'default';
    const current = getGistConfig(accountKey);

    const token = prompt(
      `GitHub token を入力してください（fine-grained token / Gists: write）\n対象アカウント: ${accountName}`,
      current.token || ''
    );
    if (token === null) return null;

    const gistId = prompt(
      `Gist ID を入力してください（初回保存で自動作成する場合は空欄のままでOK）\n対象アカウント: ${accountName}`,
      current.gistId || ''
    );
    ...
    const filename = prompt(
      `Gist 内のファイル名\n対象アカウント: ${accountName}`,
      current.filename || GIST_DEFAULT_FILENAME
    );
    if (filename === null) return null;

    const description = prompt(
      `Gist の説明\n対象アカウント: ${accountName}`,
      current.description || GIST_DEFAULT_DESCRIPTION
    );
    if (description === null) return null;

    return saveGistConfig({ token, gistId, filename, description }, accountKey);
  }

  function readLocalSyncValue(key) {
  ...
  function buildLocalSyncData() {
    const data = {};
    for (const key of GIST_SYNC_KEYS) {
      data[key] = readLocalSyncValue(key);
    }
    return data;
  }
  ...
  function normalizeBackupPayload(payload) {
    ...
    return {
      schema: 'donguri-equip-backup-v1',
      data: normalized
    };
  }
  ...
  function buildGistPayload() {
    return {
      ...
      data: buildLocalSyncData()
    };
  }

  function applyGistPayload(payload) {
    const normalized = normalizeBackupPayload(payload).data;
    for (const key of GIST_SYNC_KEYS) {
      writeLocalSyncValue(key, normalized[key]);
    }
  }
  ...
  function gistRequest({ method, path, token = '', data = null }) {
      ...
      GM_xmlhttpRequest({
        ...
        onload: (res) => {
          ...
          reject(
            new Error(
              `HTTP ${res.status}: ${
                (parsed && parsed.message) || res.responseText || 'request failed'
              }`
            )
          );
        },
  ...
  async function fetchGistFile(config) {
    let cfg = normalizeGistConfig(config);
    if (!cfg.gistId) {
    ...
    let res;
    try {
      res = await gistRequest({
        method: 'GET',
        path: `/gists/${encodeURIComponent(cfg.gistId)}`,
        token: cfg.token
      });
    } catch (e) {
      if (!isGistAuthError(e)) throw e;

      const updated = promptAndUpdateGistToken(cfg.accountKey, e.message || String(e));
      if (!updated?.token) throw e;

      cfg = normalizeGistConfig(updated);
      res = await gistRequest({
        method: 'GET',
        path: `/gists/${encodeURIComponent(cfg.gistId)}`,
        token: cfg.token
    ...
    const targetFile = files[cfg.filename] || Object.values(files)[0];

    if (!targetFile) {
      return { text: null, json: res.json, file: null };
    }

    if (targetFile.truncated) {
      throw new Error('Gist ファイルが大きすぎて API 応答で省略されています');
    }

    return {
      text: typeof targetFile.content === 'string' ? targetFile.content : null,
      json: res.json,
      file: targetFile
    };
  }

  async function savePayloadToGist(config, payload) {
    let cfg = normalizeGistConfig(config);
    if (!cfg.token) {
    ...
    const requestOnce = async () => {
      if (cfg.gistId) {
        return await gistRequest({
          method: 'PATCH',
          path: `/gists/${encodeURIComponent(cfg.gistId)}`,
          token: cfg.token,
          data: {
            files: {
              [cfg.filename]: { content: text }
            }
          }
        });
      } else {
        return await gistRequest({
          ...
          token: cfg.token,
          data: {
            description: cfg.description || GIST_DEFAULT_DESCRIPTION,
            public: false,
            files: {
              [cfg.filename]: { content: text }
            }
    ...
    const nextCfg = saveGistConfig({
      ...cfg,
      gistId: (res.json && res.json.id) ? String(res.json.id) : cfg.gistId
    });
  ...
  function startAccountCaptureBootstrap() {
    const tryCapture = () => {
      try {
        captureDetectedGistAccountName();
      } catch (e) {}
    };
    ...
    const timer = setInterval(() => {
      tryCapture();
      count += 1;
      if (count >= maxCount || getDetectedGistAccountNameCache()) {
        clearInterval(timer);
      }
    }, 1000);

    window.addEventListener('pageshow', tryCapture, { once: true });
    document.addEventListener('visibilitychange', () => {
      if (!document.hidden) tryCapture();
    });
    window.addEventListener('focus', tryCapture);
  }

  startAccountCaptureBootstrap();

  const isBattlePage =
    location.href === 'https://donguri.5ch.io/teambattle?m=hc' ||
    location.href === 'https://donguri.5ch.io/teambattle?m=l' ||
    location.href === 'https://donguri.5ch.io/teambattle?m=rb';

  const isBagPage = location.href === 'https://donguri.5ch.io/bag';

  if (!isBattlePage && !isBagPage) {
    return;
  }
  ...
  (()=>{
    ...
    menuButton.addEventListener('click', ()=>{
      ...
      requestAnimationFrame(() => {
        if (typeof syncAutoJoinDialogLayout === 'function') {
          syncAutoJoinDialogLayout();
        }
      });
    })
    ...
    (()=>{
      ...
      const autoJoinToolbarSpacer = document.createElement('div');
      autoJoinToolbarSpacer.classList.add('auto-join-toolbar-spacer');
      autoJoinToolbarSpacer.style.display = 'none';
      autoJoinToolbarSpacer.style.flex = '0 0 auto';
      autoJoinToolbarSpacer.style.width = '100%';
      autoJoinToolbarSpacer.style.pointerEvents = 'none';

      function syncAutoJoinDialogLayout() {
        const content = autoJoinDialog.querySelector('.auto-join-content');
        if (!content || !autoJoinDialog.contains(toolbar)) return;
        ...
        autoJoinToolbarSpacer.style.display = '';
        autoJoinToolbarSpacer.style.height = `${toolbarHeight}px`;

        content.style.marginTop = '0';
        content.style.height = `calc(100% - ${toolbarHeight}px)`;
        content.style.minHeight = '0';

        const log = content.querySelector('.auto-join-log');
        if (log) {
      ...
      function attachToolbarToAutoJoinDialog() {
        ...
        const content = autoJoinDialog.querySelector('.auto-join-content');
        if (content && autoJoinToolbarSpacer.parentNode !== autoJoinDialog) {
          autoJoinDialog.insertBefore(autoJoinToolbarSpacer, content);
        }

        toolbar.style.zIndex = '10';
        autoJoinDialog.style.paddingTop = '0';
        autoJoinDialog.style.overflow = 'hidden';
      ...
      function detachToolbarFromAutoJoinDialog() {
        ...
        if (autoJoinToolbarSpacer.parentNode) {
          autoJoinToolbarSpacer.parentNode.removeChild(autoJoinToolbarSpacer);
        }
      }
      ...
      (()=>{ // autoJoinSettingsDialog
       ...
       const description = document.createElement('p');
        ...
        description.innerText =
          ...
          'ume開始時刻 / ume終了時刻 で、umeR→counter→Late counter に入る時間帯を設定できます（初期値 02:00～08:30、24:00またぎ可）。\n' +
          '除外設定が有効になるのは umeR→counter と std→counter の counter モード時のみで、std には影響しません。\n' +
      ...
      (()=>{ 
        ...
        container.classList.add('auto-join-content');
        ...
        log.style.paddingTop = '1px';
        log.style.scrollPaddingTop = '1px';
  ...
  (()=>{
    ...
    (()=>{
      ...
      */
      ...
      (()=>{ 
        ...
        statusWrap.style.margin = '0 0 6px 0';
        statusWrap.style.textAlign = 'left';
        ...
        statusBadge.style.minWidth = '4.5em';
        statusBadge.style.padding = '2px 8px';
        statusBadge.style.borderRadius = '999px';
        statusBadge.style.fontSize = '75%';
        statusBadge.style.fontWeight = '700';
        statusBadge.style.border = 'solid 1px #999';
        statusBadge.style.color = '#fff';
        statusBadge.style.lineHeight = '1.2';

        const statusText = document.createElement('span');
        statusText.style.fontSize = '80%';
        statusText.style.whiteSpace = 'pre-wrap';
        statusText.style.wordBreak = 'break-word';
        statusText.style.color = '#333';

        statusWrap.append(statusBadge, statusText);

        function setSyncBadge(kind, badgeText, detailText) {
          statusBadge.textContent = badgeText;
          statusText.textContent = detailText;

          if (kind === 'green') {
            statusBadge.style.background = '#2a6';
            statusBadge.style.borderColor = '#285';
            statusText.style.color = '#2a6';
          } else if (kind === 'yellow') {
            statusBadge.style.background = '#d9a300';
            statusBadge.style.borderColor = '#b88700';
            statusText.style.color = '#9a6a00';
          } else {
            statusBadge.style.background = '#c44';
            statusBadge.style.borderColor = '#a33';
        ...
        function formatEquipPresetsTextFromObject(json) {
          ...
          const sortedEntries = Object.entries(json || {}).sort(([keyA], [keyB]) => {
          ...
          const formattedString = sortedEntries
            .map(([key, value]) => {
        ...
        function refreshBackupTextarea() {
          ...
          if (data) {
            try {
              textarea.value = formatEquipPresetsTextFromObject(JSON.parse(data));
            } catch (e) {
              textarea.value = data;
            }
          } else {
        ...
        async function refreshBackupGistStatus() {
          const localPayload = normalizeBackupPayload(buildGistPayload());
          const localCount = Object.keys((localPayload.data && localPayload.data.equipPresets) || {}).length;
          const accountLabel = getCurrentGistAccountLabel();
          const cfg = getGistConfig();

          if (!cfg.token) {
            setSyncBadge('red', '赤', `acct: ${accountLabel} / GitHub token 未設定 / ローカル ${localCount}件`);
            return;
          }

          if (!cfg.gistId) {
            setSyncBadge('yellow', '黄', `acct: ${accountLabel} / Gist 未作成 / 「Gistへ保存」で自動作成 / ローカル ${localCount}件`);
            return;
          }

          setSyncBadge('yellow', '...', `acct: ${accountLabel} / Gist 確認中...`);

          try {
            const remote = await fetchGistFile(cfg);

            if (!remote.text) {
              setSyncBadge('yellow', '黄', `acct: ${accountLabel} / Gist はあるがファイルなし / ${cfg.filename}`);
              return;
            }

            let remotePayload;
            try {
              remotePayload = normalizeBackupPayload(JSON.parse(remote.text));
            } catch (e) {
              setSyncBadge('red', '赤', `acct: ${accountLabel} / Gist JSON エラー / ${cfg.filename}`);
              return;
            }

            const remoteCount = Object.keys((remotePayload.data && remotePayload.data.equipPresets) || {}).length;

            if (stableStringify(localPayload) === stableStringify(remotePayload)) {
              setSyncBadge('green', '緑', `acct: ${accountLabel} / Gist一致 / ローカル ${localCount}件 / Gist ${remoteCount}件`);
            } else {
              setSyncBadge('yellow', '黄', `acct: ${accountLabel} / 差分あり / ローカル ${localCount}件 / Gist ${remoteCount}件`);
            }
          } catch (e) {
            setSyncBadge('red', '赤', `acct: ${accountLabel} / Gist取得失敗 / ${e.message || e}`);
          }
        }
        ...
        const gistSettingsButton = button.cloneNode();
        gistSettingsButton.textContent = 'Gist設定';
        gistSettingsButton.style.background = '#888';
        gistSettingsButton.style.color = '#fff';
        gistSettingsButton.addEventListener('click', async()=>{
          const cfg = promptAndSaveGistConfig();
          if (!cfg) return;
          await refreshBackupGistStatus();
          alert(
            `Gist設定を保存しました\n` +
            `account: ${getCurrentGistAccountLabel()}\n` +
            `file: ${cfg.filename}\n` +
            `${cfg.gistId ? `gistId: ${cfg.gistId}` : 'gistId: 未作成'}`
          );
        });

        const gistIdCopyButton = button.cloneNode();
        gistIdCopyButton.textContent = 'gistId\nコピー';
        gistIdCopyButton.style.background = '#6a6';
        gistIdCopyButton.style.color = '#fff';
        gistIdCopyButton.addEventListener('click', async()=>{
          const cfg = getGistConfig();
          if (!cfg.gistId) {
            alert(`gistId が未設定です\naccount: ${getCurrentGistAccountLabel()}`);
            return;
          ...
          try {
            ...
            alert(`gistId をコピーしました\naccount: ${getCurrentGistAccountLabel()}\n${cfg.gistId}`);
          } catch (e) {
            alert(`gistId コピー失敗: ${e}`);
          }
        });

        const gistSaveButton = button.cloneNode();
        gistSaveButton.textContent = 'Gistへ\n保存';
        gistSaveButton.style.background = '#ffb300';
        gistSaveButton.style.color = '#000';
        gistSaveButton.addEventListener('click', async()=>{
          let cfg = getGistConfig();
          if (!cfg.token) {
            cfg = promptAndSaveGistConfig();
            if (!cfg) return;
          }

          try {
            const result = await savePayloadToGist(cfg, buildGistPayload());
            await refreshBackupGistStatus();
            alert(`Gist保存完了\naccount: ${getCurrentGistAccountLabel()}\n${result.config.gistId}`);
          } catch (e) {
            alert(`Gist保存失敗: ${e.message || e}`);
          }
        });

        const gistLoadButton = button.cloneNode();
        gistLoadButton.textContent = 'Gistから\n読込';
        gistLoadButton.style.background = '#46f';
        gistLoadButton.style.color = '#fff';
        gistLoadButton.addEventListener('click', async()=>{
          let cfg = getGistConfig();
          if (!cfg.gistId) {
            cfg = promptAndSaveGistConfig();
          ...
          try {
            const remote = await fetchGistFile(cfg);
            if (!remote.text) {
              alert(`Gist に ${cfg.filename} がありません`);
              return;
            }

            let payload;
            try {
              payload = JSON.parse(remote.text);
            } catch (e) {
              alert('Gist の JSON が壊れています');
              return;
            }

            const ok = confirm('Gist内容で現在のローカル装備情報を上書きしますか？');
            if (!ok) return;

            applyGistPayload(payload);
            refreshBackupTextarea();
            await refreshBackupGistStatus();

            if (typeof showEquipPreset === 'function') {
              try { showEquipPreset(); } catch (e) {}
            }

            alert('Gist内容をローカルへ読み込みました');
          } catch (e) {
            alert(`Gist読込失敗: ${e.message || e}`);
          }
        });
        ...
        const resetBackupButton = button.cloneNode();
        resetBackupButton.textContent = 'リセット';
        resetBackupButton.addEventListener('click', ()=>{
          const ok = confirm('装備プリセットのバックアップをリセットしますか？');
          if (!ok) return;
          localStorage.removeItem('equipPresets');
          localStorage.removeItem('autoEquipItems');
          localStorage.removeItem('autoEquipItemsAutojoin');
          textarea.value = '';
          if (typeof showEquipPreset === 'function') {
            try { showEquipPreset(); } catch (e) {}
          }
          const stat = document.querySelector('.equip-preset-stat');
          if (stat) stat.textContent = '装備プリセットのバックアップをリセット';
        })
        ...
        backupDialog.append(statusWrap, textarea, div);
      ...
      accountBar.style.gap = '6px';
      accountBar.style.margin = '0 0 2px 0';
      accountBar.style.padding = '2px 6px';
      accountBar.style.border = 'solid 1px #bbb';
      accountBar.style.background = '#eef7ff';
      accountBar.style.color = '#333';

      const manualAccountInfo = document.createElement('div');
      manualAccountInfo.style.fontSize = '12px';
      manualAccountInfo.style.lineHeight = '1.2';
      manualAccountInfo.style.whiteSpace = 'nowrap';
      manualAccountInfo.style.overflow = 'hidden';
      manualAccountInfo.style.textOverflow = 'ellipsis';
      manualAccountInfo.style.flex = '1 1 auto';
      manualAccountInfo.classList.add('equip-manual-account-info');
      manualAccountInfo.textContent = getManualAccountDisplayText();

      accountBar.append(manualAccountInfo);
      ...
      (()=>{ 
        const div = document.createElement('div');
        div.classList.add('equip-top-buttons');
        div.style.display = 'flex';
        ...
        topRow.append(div, closeButton);

        buttonsContainer.append(accountBar, topRow);
      })();
    ...
    (()=>{ 
      ...
      presetNameInput.addEventListener('keydown', (e)=>{
        if (e.key === "Enter") {
          e.preventDefault();
          const autoName = applyAutoPresetNameToInput(presetNameInput);
          const presetName = ((presetNameInput.value || '').trim() || autoName || '').substring(0,32);
    ...
    function colorizeEquipElementCell(cell) {
      ...
      const elem = (raw.match(/[^\d]+$/) || ['無'])[0].trim();
    ...
    function getStoredHeaderSort(table) {
      if (!table) return null;

      const kind = table.dataset.sortKind || '';
      const columnIndex = Number(table.dataset.sortColumnIndex);
      const direction = table.dataset.sortDirection || '';

      if (!kind || !Number.isFinite(columnIndex) || !direction) return null;
      return { kind, columnIndex, direction };
    }
    ...
    function sortTable(table, mode = currentEquipSort){
      ...
      const headerMode = mode === 'header' ? getStoredHeaderSort(table) : null;
  ...
  function getPresetBaseRank(key, value) {
    ...
    if (ranks.includes('GUARD_N')) return 'N';
  ...
  async function setPresetItems (presetName) {
    ...
    // 装備パネル未描画時に null.textContent で落ちるのを防ぐ
    if (!stat) {
      console.warn('[setPresetItems] .equip-preset-stat not found');
      return false;
    }
  ...
  async function autoJoin() {
    ...
    function isMorningTime() {
      ...
      return currentMinutes >= startMinutes || currentMinutes <= endMinutes;
        ...
        let suppressCounterNoTargetLog = false;
        let stdCounterActive = false;
        let lateCounterStartLogged = false;
    ...
    function shouldStopLateCounterMonitor(progress) {
      return !(
        (progress >= 48 && progress <= 50) ||
        (progress >= 98 && progress <= 100)
      );
    ...
    function getLateCounterMonitorMs(progress) {
      if ((progress >= 49 && progress <= 50) || (progress >= 99 && progress <= 100)) {
        return 1000;
      }
      return counterMonitorMs;
    }

    function shouldStartStdCounter(progress) {
          return (progress >= 20 && progress <= 47) || (progress >= 70 && progress <= 97);
        }

        function getStdNextProgress(progress) {
          if (progress < 3) {
            return 3;
          } else if (progress < 18) {
            return 18;
        ...
        function getStdNextProgressAfterSuccess(progress) {
          if (progress < 17) {
            return 18;
          } else if (progress < 52) {
            return 53;
    ...
    function logMessage(region, message, next) {
      ...
      const attackmode =
        autoJoinMode === 'lateCounter' ? 'lateCounter' :
        autoJoinMode === 'counter' ? 'counter' :
        autoJoinMode === 'umeR' ? 'umeR' : 'stdP';
    ...
    async function attackRegion () {
      ...
      const forceStdCounter =
        autoJoinMode === 'stdP' &&
        !isMorningTime() &&
        shouldStartStdCounter(currentProgress);

      const forceLateCounterWindow =
        autoJoinMode === 'stdP' &&
        !isMorningTime() &&
        shouldStopCounterMonitor(currentProgress);

      if (isAutoJoinRunning || (!(forceStdCounter || forceLateCounterWindow) && Math.abs(nextProgress - currentProgress) >= 2)) {
        return;
      }
    ...
    if (location.href.includes('/teambattle?m=rb')) {
      ...
      let cellType = pickCellType(regions) || 'mapEdge';
      ...
      while(dialog.open) {
        ...
        if ((autoJoinMode === 'umeR' || (autoJoinMode === 'counter' && !stdCounterActive)) && !isMorningTime()) {
          autoJoinMode = 'stdP';
          nextProgress = getStdNextProgress(currentProgress);
          ...
          cellType = pickCellType(regions) || 'mapEdge';
          restartAfterLoop = true;
          logMessage(null, '[切替] umeR/counter → stdP', '→ 時間終了');
        } else if (autoJoinMode === 'stdP' && isMorningTime()) {
          autoJoinMode = 'umeR';
          stdCounterActive = false;
          counterMonitorLogged = false;
          suppressCounterNoTargetLog = false;
          ...
          cellType = pickCellType(regions) || 'mapEdge';
          restartAfterLoop = true;
          logMessage(null, '[切替] stdP → umeR', '→ 時間開始');
        } else if (autoJoinMode === 'stdP' && !isMorningTime() && shouldStopCounterMonitor(currentProgress)) {
          autoJoinMode = 'lateCounter';
          stdCounterActive = false;
          counterMonitorLogged = false;
          suppressCounterNoTargetLog = false;
          lateCounterStartLogged = false;
          ...
          cellType = pickCellType(regions) || 'mapEdge';
          restartAfterLoop = true;
          logMessage(null, '[移行] stdP待機 → Late counterへ移行', `→ ${currentProgress}%`);
        } else if (autoJoinMode === 'stdP' && !isMorningTime() && shouldStartStdCounter(currentProgress)) {
          const prevMode = autoJoinMode;
          autoJoinMode = 'counter';
          excludeSet.clear();
          ...
          if (nextCellType) {
            ...
            suppressCounterNoTargetLog = false;
            lateCounterStartLogged = false;
            stdCounterActive = true;
            cellType = nextCellType;
            restartAfterLoop = true;
            logMessage(null, '[切替] stdP → 応戦', `→ ${currentProgress}%`);
          } else {
            autoJoinMode = prevMode;
            stdCounterActive = false;
            if (!counterMonitorLogged) {
              counterMonitorLogged = true;
              logMessage(null, '[待機] counter時間・対象なし', `→ ${counterMonitorMs / 1000}s後再確認`);
            }
          }
        }
        ...
        const isMorning = isUme || isCounterLike;

        // counter→late counter への切替を「候補ゼロ時だけ」にせず、
        // 境界到達時点で即時実行する
        if (isCounter) {
          await drawProgressBar();
          ...
          if (shouldStopCounterMonitor(currentProgress)) {
            ...
            logMessage(null, '[移行] counter監視停止 → Late counterへ移行', '→ lateCounter');
            continue;
          }
        }
        ...
        if (isCounterLike && regions[cellType].length === 0) {
          const nextCellType = pickCellType(regions);
          if (nextCellType) {
            cellType = nextCellType;
            regions[cellType] = regions[cellType]
        ...
        for (let i = 0; i < regions[cellType].length;) {
          ...
          try {
            ...
            if (messageType === 'capitalAttack') {
              if (isLateCounter) {
                message = '[Late counter] 首都は攻撃しないためスキップ';
                processType = 'break';
                i++;
              } else if (isMorning) {
                if (loop < maxloop){
              ...
              if (isCounterLike) {
                message = lastLine;
                counterMonitorLogged = true;
                suppressCounterNoTargetLog = true;

                excludeSet.clear();
                regions = await getRegions();
                cellType = pickCellType(regions, cellType) || 'mapEdge';
                restartAfterLoop = true;
                sleepTime = counterMonitorMs / 1000;
                processType = 'reload';
              } else if (isMorning) {
                message = lastLine;
                excludeSet.delete(region.join(','));
              ...
              const isOverMod = lastLine.includes('装備しているものは改造が多すぎます。改造の少ない他のものをお試しください');
            ...
            if (success) {
              if (location.href.includes('/teambattle?m=rb')) {
                ...
                if (isMorning) {
                  ...
                  nextProgress = getStdNextProgressAfterSuccess(currentProgress);
                }
              }
              next = `→ ${nextProgress}±1%`;
              isAutoJoinRunning = false;
//            logMessage(null, '[打止] 終了です。', next);
        ...
        if (!success && regions[cellType].length === 0) {
          if (isCounter) {
            ...
            if (skippedCount > 0 && !counterMonitorLogged) {
              logMessage(null, `[監視] counter除外でスキップ (${skippedCount}件)`, `→ ${counterMonitorMs / 1000}s`);
            }
          ...
          if (isUme) {
            ...
            cellType = pickCellType(regions) || 'mapEdge';
            isAutoJoinRunning = false;
            logMessage(null, '[移行] umeR打止 → 応戦モードへ移行', '→ counter');
            continue;
          }
    ...
    async function getRegions () {
      try {
        ...
        if (isUme || isCounterLike) {
          ...
          let nonAdjacentSafe     = await filterGuardCells(filteredCells(nonAdjacentBase));
          let capitalAdjacentSafe = await filterGuardCells(filteredCells(capitalAdjacentCells));
          let teamAdjacentSafe    = await filterGuardCells(filteredCells(teamAdjacentCells));
          let mapEdgeSafe         = await filterGuardCells(filteredCells(mapEdgeCells));
          let onceMoreSafe        = await filterGuardCells(filteredCells(onceMoreBase));

          let counterExcludedSkipped = 0;
          let lateCounterEnemyCount = 0;

          if (isCounterLike) {
            const counterExcludedColors = new Set(
              [...getCounterExcludedColorSet()].map(v => String(v).toUpperCase())
            );
            const teamColorUpper = String(teamColor || '').toUpperCase();

            lateCounterEnemyCount = cells.filter(([r, c]) => {
              const key = `${r}-${c}`;
              const color = cellColors[key]?.replace('#', '').toUpperCase();
              return !!color && color !== teamColorUpper;
            }).length;

            const filterCounterExcludedTargets = (targets) => {
              return targets.filter(([r, c]) => {
                const key = `${r}-${c}`;
                const color = cellColors[key]?.replace('#', '').toUpperCase();

                if (!color) return true;
                if (color === teamColorUpper) return true;

                const shouldSkip = counterExcludedColors.has(color);
                if (shouldSkip) counterExcludedSkipped += 1;
                return !shouldSkip;
              });
            };

            nonAdjacentSafe     = filterCounterExcludedTargets(nonAdjacentSafe);
            capitalAdjacentSafe = filterCounterExcludedTargets(capitalAdjacentSafe);
            teamAdjacentSafe    = filterCounterExcludedTargets(teamAdjacentSafe);
            mapEdgeSafe         = filterCounterExcludedTargets(mapEdgeSafe);
            onceMoreSafe        = filterCounterExcludedTargets(onceMoreSafe);

            if (isLateCounter) {
              // 同じチーム（同色）の敵マス総数を、首都込みで数える
              // ただし自軍色と counter除外色は対象外
              const enemyCountByColor = new Map();
              ...
              cells.forEach(([r, c]) => {
                ...
                if (!color) return;
                if (color === teamColorUpper) return;
                if (counterExcludedColors.has(color)) return;

                enemyCountByColor.set(color, (enemyCountByColor.get(color) || 0) + 1);
              });

              // 「同じチームの敵マスが2つ以上」の色だけを late counter 対象にする
              const lateCounterTargetColors = new Set(
                [...enemyCountByColor.entries()]
                  .filter(([, count]) => count >= 2)
                  .map(([color]) => color)
              );

              // ログ用：条件を満たす敵マス総数
              lateCounterEnemyCount = [...enemyCountByColor.entries()]
                .filter(([, count]) => count >= 2)
                .reduce((sum, [, count]) => sum + count, 0);

              const filterLateCounterTargets = (targets) => {
                if (lateCounterTargetColors.size === 0) return [];

                return targets.filter(([r, c]) => {
                  const key = `${r}-${c}`;
                  if (capitalSet.has(key)) return false; // 首都は撃たない

                  const color = cellColors[key]?.replace('#', '').toUpperCase();
                  return !!color && lateCounterTargetColors.has(color);
                });
              };

              // late counterでも首都以外の敵を全カテゴリから拾う
              nonAdjacentSafe     = filterLateCounterTargets(nonAdjacentSafe);
              capitalAdjacentSafe = filterLateCounterTargets(capitalAdjacentSafe);
              teamAdjacentSafe    = filterLateCounterTargets(teamAdjacentSafe);
              mapEdgeSafe         = filterLateCounterTargets(mapEdgeSafe);
              onceMoreSafe        = filterLateCounterTargets(onceMoreSafe);
            }
          }

          regions = {
            nonAdjacent: shuffle(nonAdjacentSafe),
    ...
    autoJoinMode = isMorningTime() ? 'umeR' : 'stdP';
  ...
  async function drawProgressBar(){
    try {
      ...
      const containerSrc = doc.querySelector('div.stat-block:nth-child(2)>div:nth-child(5)');
      const statBlock = doc.querySelector('.stat-block');

      // マップ更新直後などでトップページのDOMが欠けることがある
      if (!containerSrc || !statBlock) {
        console.warn('[drawProgressBar] progress container/stat-block not found');
        return;
      }

      const container = containerSrc.cloneNode(true);
      const periodText = container.firstChild?.textContent || '';
      const progressText = container.lastElementChild?.textContent || '';

      const periodMatch = periodText.match(/\d+/);
      const parsedProgress = parseInt(progressText, 10);

      if (!periodMatch || Number.isNaN(parsedProgress)) {
        console.warn('[drawProgressBar] invalid progress text', { periodText, progressText });
        return;
      }

      currentPeriod = Number(periodMatch[0]);
      ...
      const statText = statBlock.textContent || '';
      const woodMatch = statText.match(/木材の数: (\d+)/);
      const steelMatch = statText.match(/鉄の数: (\d+)/);
