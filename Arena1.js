...
(function () {
  ...
  function openManualChallengePopup() {
    const name = `dar_manual_challenge_popup_${Date.now()}`;
    const features = [
      'width=900',
      'height=900',
      'left=80',
    ...
    try {
      ...
      if (popup) {
        ...
        popup.document.write(`
          ...
          <body>
            <div class="dar-loading">対戦ログを読み込み中です…</div>
          </body>
          </html>
        `);
  ...
  async function handleChallengePage() {
    // challenge結果ページでは、pending が欠けても手動ログ消しボタンだけは出す
  ...
  async function handleArenaPage() {
    ...
    const heldRow = targetRows.find(r => isHeldRegion(r, state, settings));
    if (heldRow) {
      await saveState({
        ...state,
        monitoringRegions: selectedRegions,
        lastSubmitAt: 0,
        lastResult: `[監視中] ${selectedRegions.join(' / ')} / 保持=${heldRow.leader || 'なし'}`
      });
      await refreshPanel(rows, heldRow, await loadState(), settings);
      return;
    }

    const attackableTargets = targetRows.filter(r => !isHeldRegion(r, state, settings));
    const derivedTurn = currentTurnName(state, settings, rows);

    if (!isMyTurn(state, settings, rows)) {
      await saveState({
        ...state,
        monitoringRegions: selectedRegions,
        lastSubmitAt: 0,
        lastResult: `[待機] 次担当=${derivedTurn || '(なし)'}`
      });
      await refreshPanel(rows, primaryTarget, await loadState(), settings);
      return;
    }

    if (attackableTargets.length > 0) {
      await refreshPanel(rows, attackableTargets[0], state, settings);
  ...
  function isManualChallengePopup() {
    return location.pathname.startsWith('/challenge') && !!window.opener;
  }

  async function main() {
    clearArenaReload();

    if (isManualChallengePopup()) {
