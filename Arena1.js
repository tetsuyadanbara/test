...
(function () {
  ...
  async function gmGetValueCompat(key, defaultValue) {
    try {
      ...
      console.warn('[DAR-REWRITE] getValue error', error);
    }

    console.warn('[DAR-REWRITE] getValue API is unavailable; using in-page fallback');
    try {
      const raw = localStorage.getItem(`dar_gm_fallback__${key}`);
  ...
  async function gmSetValueCompat(key, value) {
    try {
      ...
      console.warn('[DAR-REWRITE] setValue error', error);
    }

    console.warn('[DAR-REWRITE] setValue API is unavailable; using in-page fallback');
    try {
      localStorage.setItem(`dar_gm_fallback__${key}`, JSON.stringify(value));
      return true;
    } catch (error) {
      console.warn('[DAR-REWRITE] local fallback setValue error', error);
      return false;
    }
  }

  async function loadState() {
    const state = await gmGetValueCompat(SHARED_KEY, defaultState());
  ...
  async function saveState(patch) {
    ...
    await gmSetValueCompat(SHARED_KEY, next);
    return next;
  }
  ...
  function addSharedStateListener(handler) {
    try {
      if (typeof GM_addValueChangeListener === 'function') {
        return GM_addValueChangeListener(SHARED_KEY, handler);
      }
      if (typeof GM !== 'undefined' && typeof GM.addValueChangeListener === 'function') {
        return GM.addValueChangeListener(SHARED_KEY, handler);
      }
    } catch (error) {
      console.warn('[DAR-REWRITE] addValueChangeListener error', error);
    }

    console.warn('[DAR-REWRITE] value change listener is unavailable; fallback to normal polling only');
    return null;
  }

  function removeSharedStateListener(listenerId) {
    try {
      ...
      if (typeof GM !== 'undefined' && typeof GM.removeValueChangeListener === 'function') {
        GM.removeValueChangeListener(listenerId);
      }
    } catch (error) {
      console.warn('[DAR-REWRITE] removeValueChangeListener error', error);
    }
  }

  const listenerId = addSharedStateListener(async (_key, _oldValue, _newValue, remote) => {
    if (!remote) return;
  ...
  window.addEventListener('beforeunload', () => {
    clearArenaReload();
    removeSharedStateListener(listenerId);
  });
