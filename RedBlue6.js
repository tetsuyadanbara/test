...
(()=>{
  ...
  async function autoJoin() {
    ...
    async function fetchBattleState() {
      ...
      const buildingsMap = Object.create(null);
        ...
        try {
          ...
          if (Array.isArray(buildingsPayload.buildings)) {
            buildingsPayload.buildings.forEach(item => {
              const rc = Array.isArray(item?.rc)
                ? item.rc
                : [item?.r ?? item?.row, item?.c ?? item?.col];
              ...
              const kindRaw = String(
                b.kind ?? b.type ?? b.buildingType ?? b._buildingType ?? b.k ?? ''
              ).toLowerCase();

              let kind = '';
              if (kindRaw === 'f' || kindRaw.includes('fort') || kindRaw.includes('wall')) kind = 'f';
              else if (kindRaw === 'r' || kindRaw.includes('radar')) kind = 'r';
              if (!kind) return;

              let team = Number(b.team ?? b.owner ?? b.ownerTeam ?? b.t ?? 0);
              if (team !== 1 && team !== 2) {
                const teamRaw = String(b.team ?? b.owner ?? b.ownerTeam ?? b.t ?? '').toLowerCase();
                if (teamRaw.includes('red')) team = 1;
                else if (teamRaw.includes('blue')) team = 2;
                else team = teamMap[keyOf(r, c)] || 0;
              }

              buildingsMap[keyOf(r, c)] = { kind, team };
            });
          }
        } catch (_) {}
      }
    ...
    const AUTO_FORT_OPENING_EMPTY_TARGET = 2;
    const AUTO_FORT_TARGET = 3;
    const AUTO_FORT_COOLDOWN_MS = 12000;

    function getAutoFortPlanKey() {
      const stage = location.href.includes('/teambattle?m=rb') ? (getCurrentRBStage() ?? '') : '';
      return `${MODE}|${currentPeriod ?? ''}|${stage}`;
    }

    function syncAutoFortPlanState() {
      const key = getAutoFortPlanKey();
      if (autoFortPlanKey === key) return;
      autoFortPlanKey = key;
      autoFortPlanPhase = 'opening';
      autoFortOpeningMoves = 0;
      lastAutoFortifyAt = 0;
    }

    function getCapitalUsableNeighbors(state, ownCapital) {
      if (!ownCapital) return [];
      return orthNeighbors(state, ownCapital)
        .filter(v => isExplorable(state, v.r, v.c))
        .filter(v => !isWater(state, v.r, v.c))
        .filter(v => isFriendly(state, v.r, v.c));
    }

    function getCapitalFortTargetCount(state, ownCapital) {
      return Math.min(AUTO_FORT_TARGET, getCapitalUsableNeighbors(state, ownCapital).length);
    }

    function hasCapitalAdjacentEmpty(state, ownCapital) {
      return orthNeighbors(state, ownCapital).some(v => isEmpty(state, v.r, v.c));
    }

    function getFriendlyFortCountAroundCapital(state, ownCapital) {
      const buildingsMap = state.buildingsMap || Object.create(null);
      return getCapitalUsableNeighbors(state, ownCapital)
        .filter(v => {
          const b = buildingsMap[keyOf(v.r, v.c)];
          return b && b.kind === 'f' && (b.team === state.myTeamCode || b.team === 0);
        })
        .length;
    }
    ...
    async function autoBuildNextFortAroundCapital(state, targetCount) {
      ...
      const usableNeighbors = getCapitalUsableNeighbors(latestState, ownCapital);
      const normalizedTarget = Math.min(targetCount, usableNeighbors.length);
      const currentFortCount = getFriendlyFortCountAroundCapital(latestState, ownCapital);

      if (normalizedTarget <= 0 || currentFortCount >= normalizedTarget) {
        return {
          stop: false,
          state: latestState,
          built: 0,
      ...
      const cooldownLeftMs = AUTO_FORT_COOLDOWN_MS - (Date.now() - lastAutoFortifyAt);
      if (lastAutoFortifyAt > 0 && cooldownLeftMs > 0) {
        return {
          stop: false,
          state: latestState,
          built: 0,
          attempted: 0,
          waitCooldown: true,
          waitSeconds: Math.max(1, Math.ceil(cooldownLeftMs / 1000)),
          message: ''
        };
      }

      const buildingsMap = latestState.buildingsMap || Object.create(null);
      const candidates = usableNeighbors.filter(v => !buildingsMap[keyOf(v.r, v.c)]);
      ...
      for (const cell of candidates) {
        ...
        if (ok) {
          lastAutoFortifyAt = Date.now();
          await sleep(700);
          latestState = await fetchBattleState();
          const latestCapital = findOwnCapital(latestState) || ownCapital;
          const afterFortCount = getFriendlyFortCountAroundCapital(latestState, latestCapital);
          ...
          return {
            ...
            waitCooldown: false,
            waitSeconds: 0,
            message: `[要塞] 首都周囲の要塞を ${afterFortCount}/${normalizedTarget} にしました。`
          };
        }

        if (
          typeof buildMessage === 'string' &&
          (buildMessage.includes('行動回数') || buildMessage.includes('しばらくお待ちください'))
        ) {
          lastAutoFortifyAt = Date.now();
          return {
            stop: false,
            ...
            attempted,
            waitCooldown: true,
            waitSeconds: Math.max(1, Math.ceil(AUTO_FORT_COOLDOWN_MS / 1000)),
            message: ''
          };
        }
    ...
    async function planAndAct(stateOverride = null) {
      ...
      syncAutoFortPlanState();

      if (ownCapital && autoFortPlanPhase === 'opening') {
        const openingFortResult = await autoBuildNextFortAroundCapital(state, 1);

        if (openingFortResult.stop) {
          return openingFortResult;
        }

        state = openingFortResult.state || state;
        pos = currentPos(state);

        if (openingFortResult.built > 0) {
          if (openingFortResult.message) {
            logMessage(null, openingFortResult.message, '');
          }
          autoFortPlanPhase = 'openingEmpty';
          return {
            acted: false,
            stop: false,
            keepWatching: true,
            waitSeconds: 2,
            message: '初動要塞の設置後、空白マス確保へ移ります。',
            state,
            phase: 'fort'
          };
        }

        if (openingFortResult.waitCooldown) {
          return {
            acted: false,
            stop: false,
            keepWatching: true,
            waitSeconds: openingFortResult.waitSeconds || 3,
            message: '要塞クールダウン待機中です。',
            state,
            phase: 'fort'
          };
        }
      ...
      if (ownCapital && autoFortPlanPhase === 'openingEmpty') {
        const openingPriorityPath = bfsToPriorityEmpty(state, pos);
        if (openingPriorityPath && openingPriorityPath.length >= 2) {
          const result = await followPath(openingPriorityPath, state);
          const nextState = result.state || state;

          if (result.moved) {
            autoFortOpeningMoves += 1;
            const latestCapital = findOwnCapital(nextState) || ownCapital;
            if (
              autoFortOpeningMoves >= AUTO_FORT_OPENING_EMPTY_TARGET ||
              !hasCapitalAdjacentEmpty(nextState, latestCapital)
            ) {
              autoFortPlanPhase = 'fortify';
            }
          }
        ...
        const openingEmptyPath = bfsToNearestEmpty(state, pos);
        if (openingEmptyPath && openingEmptyPath.length >= 2 && autoFortOpeningMoves < AUTO_FORT_OPENING_EMPTY_TARGET) {
          const result = await followPath(openingEmptyPath, state);
          const nextState = result.state || state;
      ...
      if (ownCapital && autoFortPlanPhase === 'fortify') {
        const fortTargetCount = getCapitalFortTargetCount(state, ownCapital);
        const fortifyResult = await autoBuildNextFortAroundCapital(state, fortTargetCount);
        ...
        const latestCapital = findOwnCapital(state) || ownCapital;
        const currentFortCount = getFriendlyFortCountAroundCapital(state, latestCapital);

        if (fortifyResult.built > 0) {
          if (fortifyResult.message) {
            logMessage(null, fortifyResult.message, '');
          }

          if (currentFortCount < fortTargetCount) {
            return {
              acted: false,
              stop: false,
              keepWatching: true,
              waitSeconds: 2,
              message: '首都周囲の要塞を追加中です。',
              state,
              phase: 'fort'
            };
          }
        ...
        if (currentFortCount < fortTargetCount && fortifyResult.waitCooldown) {
          return {
            acted: false,
            stop: false,
            keepWatching: true,
  ...
  let autoFortPlanKey = '';
  let autoFortPlanPhase = 'opening';
  let autoFortOpeningMoves = 0;
  let lastAutoFortifyAt = 0;

  function resetAutoFortPlanState() {
    autoFortPlanKey = '';
    autoFortPlanPhase = 'opening';
    autoFortOpeningMoves = 0;
    lastAutoFortifyAt = 0;
