...
(()=>{
  ...
  async function autoJoin() {
    ...
    function getHomeAnchor(state) {
      ...
      if (ownCapital) {
        autoHomeAnchor = { r: ownCapital.r, c: ownCapital.c };
        return { r: ownCapital.r, c: ownCapital.c };
      }

      if (autoHomeAnchor && Number.isInteger(autoHomeAnchor.r) && Number.isInteger(autoHomeAnchor.c)) {
        return { r: autoHomeAnchor.r, c: autoHomeAnchor.c };
      }

      const pos = currentPos(state);
      if (pos) {
        autoHomeAnchor = { r: pos.r, c: pos.c };
    ...
    async function planAndAct(stateOverride = null) {
      ...
      if (!state.hasCapital) {
        ...
        if (!state.hasCapital && pos && !autoHomeAnchor) {
          autoHomeAnchor = { r: pos.r, c: pos.c };
          logMessage(`${pos.r},${pos.c}`, '[仮首都] 実首都を取得できないため現在地を基点に続行します。', '');
        }
      }
      ...
      for (let loop = 0; loop < 100; loop++) {
        ...
        if (location.href.includes('/teambattle?m=rb') && isRBStageRefreshProgress(currentProgress)) {
          return {
            ...
            keepWatching: true,
            waitSeconds: 1,
            message: '[待機] マップ更新直後のため次の1%まで戦闘開始を待機します。',
            state,
            phase: 'rbRefresh'
          };
        }
