...
(()=>{
  ...
  (()=>{
    ...
    bar.append(rankSelect, sortModeButton, equipSwitchButton, registerButton, p);
    ...
    function getEquipModValue(row) {
      const modCell = row.cells[7];
      if (!modCell) return Number.NEGATIVE_INFINITY;

      const text = modCell.textContent.replace(/,/g, '');
    ...
    function sortTable(table, mode = currentEquipSort){
      ...
      rows.sort((a,b) => {
        if (useModSort) {
          ...
          if (modA !== modB) return modB - modA; // mod大きい順
        }
