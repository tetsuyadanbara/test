...
//            logMessage(null, '[打止] 終了です。', next);
        ...
        if (!success && regions[cellType].length === 0) {
          ...
          if (isLateCounter) {
            ...
            if (!counterMonitorLogged) {
              if (lateEnemyCount < 2) {
                logMessage(null, '[監視] Late counter待機（敵2マス未満）', `→ ${counterMonitorMs / 1000}s`);
              } else if (skippedCount > 0) {
                logMessage(null, `[監視] counter除外でスキップ (${skippedCount}件)`, `→ ${counterMonitorMs / 1000}s`);
              } else {
                logMessage(null, '[監視] Late counter待機（首都以外のみ攻撃）', `→ ${counterMonitorMs / 1000}s`);
              }
            }
