...
(()=>{
  ...
  async function autoJoin() {
    ...
    let nextProgress;
    let umeRWindowKey = '';
    let umeRLeaderCount = 0;
    let umeRActiveRegionKey = null;
    let umeRWindowStopped = false;
    let umeRCompletedWindowKey = '';

    function getRestrictedUmeRWindowInfo() {
      if (MODE === 'm=rb') {
        return { active: false, key: '', nextProgress: nextProgress };
      }
      if (currentProgress >= 48 && currentProgress <= 50) {
        return { active: true, key: '48-50', nextProgress: 98 };
      }
      if (currentProgress >= 98 && currentProgress <= 100) {
        return { active: true, key: '98-100', nextProgress: 48 };
      }
      if (currentProgress < 48) {
        return { active: false, key: '', nextProgress: 48 };
      }
      if (currentProgress < 98) {
        return { active: false, key: '', nextProgress: 98 };
      }
      return { active: false, key: '', nextProgress: 48 };
    }
    ...
    async function attackRegion () {
      ...
      const isMorningAtStart = isMorningTime();
      const useRestrictedUmeR = isMorningAtStart && MODE !== 'm=rb';

      if (isAutoJoinRunning) {
        return;
      }

      if (useRestrictedUmeR) {
        const windowInfo = getRestrictedUmeRWindowInfo();
        nextProgress = windowInfo.nextProgress;
        ...
        if (umeRCompletedWindowKey === windowInfo.key) {
          umeRWindowKey = windowInfo.key;
          return;
        }

        if (umeRWindowKey !== windowInfo.key) {
          umeRWindowKey = windowInfo.key;
          umeRLeaderCount = 0;
          umeRActiveRegionKey = null;
          umeRWindowStopped = false;
        }

        if (umeRWindowStopped || umeRLeaderCount >= 2) {
          return;
        }
      } else if (Math.abs(nextProgress - currentProgress) >= 2) {
        return;
      }
    ...
    if (location.href.includes('/teambattle?m=rb')) {
      ...
      while(dialog.open) {
        ...
        const useRestrictedUmeR = isMorning && MODE !== 'm=rb';

        regions[cellType] = regions[cellType]
          .filter(e => !excludeSet.has(e.join(',')));

        if (useRestrictedUmeR && umeRActiveRegionKey) {
          regions[cellType].sort((a, b) => {
            const ak = a.join(',');
            const bk = b.join(',');
            if (ak === umeRActiveRegionKey && bk !== umeRActiveRegionKey) return -1;
            if (ak !== umeRActiveRegionKey && bk === umeRActiveRegionKey) return 1;
            return 0;
          });
        }
        ...
        for (let i = 0; i < regions[cellType].length;) {
          if (useRestrictedUmeR) {
            await drawProgressBar();
            const windowInfoNow = getRestrictedUmeRWindowInfo();
            nextProgress = windowInfoNow.nextProgress;

            if (!windowInfoNow.active) {
              umeRWindowKey = '';
              umeRWindowStopped = true;
              isAutoJoinRunning = false;
              return;
            }

            if (umeRCompletedWindowKey === windowInfoNow.key) {
              umeRWindowKey = windowInfoNow.key;
              isAutoJoinRunning = false;
              return;
            }

            if (umeRWindowKey !== windowInfoNow.key) {
              umeRWindowKey = windowInfoNow.key;
              umeRLeaderCount = 0;
              umeRActiveRegionKey = null;
              umeRWindowStopped = false;
          ...
          try {
            ...
            const isEliteCell = /e$/.test(cellRank);
            const normalizedCellRank = cellRank.replace(/e$/, '').replace(/だけ$/, '');
            const isHighRankCell = normalizedCellRank === 'SSR' || normalizedCellRank === 'UR';
            const isGuardCell = equipChangeStat === 'guardCell';

            if (isGuardCell || equipChangeStat === 'noEquip' || (useRestrictedUmeR && (isEliteCell || isHighRankCell))) {
              excludeSet.add(regionKey);
              if (useRestrictedUmeR && umeRActiveRegionKey === regionKey) {
                umeRActiveRegionKey = null;
              }
              i++;
              continue;
            } else if (useRestrictedUmeR) {
              if (!umeRActiveRegionKey) {
                umeRActiveRegionKey = regionKey;
              }
            } else {
              excludeSet.add(regionKey);
            }
            ...
            if (messageType === 'capitalAttack') {
              ...
              if (useRestrictedUmeR) {
                ...
                if (text.startsWith('リーダーになった')) {
                  ...
                  if (umeRLeaderCount >= 2) {
                    umeRCompletedWindowKey = umeRWindowKey;
                    umeRWindowStopped = true;
                    success = true;
                    message = '[2マス完了] (' + loop + '発目) ' + lastLine;
                    processType = 'return';
                  } else {
                    sleepTime = 1;
                    message = '[' + umeRLeaderCount + '/2リーダー] (' + loop + '発目) ' + lastLine;
                    processType = 'reload';
                  }
                } else {
                  sleepTime = 1;
            ...
            if (success) {
              if (useRestrictedUmeR) {
                nextProgress = currentProgress < 50 ? 98 : 48;
              } else if (location.href.includes('/teambattle?m=rb')) {
        ...
        if (!success && regions[cellType].length === 0) {
          if (isMorning && MODE !== 'm=rb') {
            ...
            if (regions.nonAdjacent.length > 0) {
              ...
              umeRActiveRegionKey = null;
              isAutoJoinRunning = false;
              logMessage(null, '[待機] 攻撃候補が見つからないため、次の確認まで待機します。', `→ ${nextProgress}±1%`);
              return;
            }

            umeRActiveRegionKey = null;
            isAutoJoinRunning = false;
            logMessage(null, '[再取得] 別の候補帯を確認します。', '→ 1s');
            await sleep(1000);
            continue;
          } else if (location.href.includes('/teambattle?m=rb')) {
            if (currentProgress < 16) {
...
//          nonAdjacent: shuffle(nonAdjacentCells),
    ...
    async function equipChange (region) {
      ...
      try {
        ...
        const isGuardCell = equipCond.includes('警備員');
        ...
        if (isGuardCell) {
          return [rank, 'guardCell'];
        } else if (autoEquipItemsAutojoin[rank]?.length > 0) {
