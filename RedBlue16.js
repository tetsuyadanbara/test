  ...
  (()=>{
    ...
    (()=>{
      ...
      function updateAutoJoinDialogPosition() {
        ...
        let widthPx = window.innerWidth - sideMargin * 2;

        // メニューバーが左側にあるときは、その右側にログを出す
        if (toolbarRect.left <= window.innerWidth / 2) {
          leftPx = Math.ceil(toolbarRect.right + 8);
          widthPx = window.innerWidth - leftPx - sideMargin;
        } else {
          // メニューバーが右側にあるときは、その左側にログを出す
          leftPx = sideMargin;
          widthPx = Math.floor(toolbarRect.left - sideMargin * 2);
        }

        if (widthPx < minWidthPx) {
          leftPx = sideMargin;
          widthPx = window.innerWidth - sideMargin * 2;
        }
