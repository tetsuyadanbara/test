...
(()=>{
  ...
  async function autoJoin() {
    ...
    function isInStartWindow(progress, target) {
      if (target === null) return true;          // 初回開始はそのまま許可
      if (target === 49) return progress >= 48 && progress < 50;   // 48,49
      if (target === 99) return progress >= 98 && progress < 100;  // 98,99
    ...
    async function attackRegion () {
      ...
      if (!isInStartWindow(currentProgress, nextProgress)) {
        return;
      }
