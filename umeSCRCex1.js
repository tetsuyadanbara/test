...
(()=>{
  ...
  async function autoJoin() {
    ...
    function parseExcludedTeamsForAutoJoin() {
      return String(settings.excludedTeams || '')
        .split(/[\n,、]+/)
        .map(v => v.trim())
        .filter(Boolean);
    ...
    function getCurrentTeamColorMapForAutoJoin() {
      const map = {};
    ...
    function getExcludedTeamColorsForAutoJoin() {
      const teamColorMap = getCurrentTeamColorMapForAutoJoin();
      return new Set(
        parseExcludedTeamsForAutoJoin()
          .map(v => {
            if (/^[0-9A-Fa-f]{6}$/.test(v)) return v.toUpperCase();
            return teamColorMap[v];
          })
    ...
    let lateCounterActive = false;
    let stdParticipationShotPending = !isMorningTime();

    function shouldStopCounterMonitor(progress) {
      return progress === 47 || progress === 97;
    }

    function isLateCounterWindow(progress) {
      return (progress >= 48 && progress < 50) || (progress >= 98 && progress < 100);
    }

    function shouldStartLateCounter(regions, progress) {
      return isLateCounterWindow(progress) && (regions?.lateCounter?.length || 0) >= 2;
    }

    function isStdParticipationPending() {
      return autoJoinMode === 'stdP' &&
        !isMorningTime() &&
        !stdCounterActive &&
        !lateCounterActive &&
        stdParticipationShotPending;
    }
    ...
    function shouldStartStdCounter(progress) {
        ...
        function pickCellType(regions, preferred = null) {
         if (lateCounterActive) {
           return regions?.lateCounter?.length > 0 ? 'lateCounter' : null;
         }
        ...
        function resolveCellType(regions, preferred = null) {
          return pickCellType(regions, preferred) || (lateCounterActive ? 'lateCounter' : 'mapEdge');
        }

        function resolveStdParticipationCellType(regions, preferred = null) {
          if (isStdParticipationPending() && regions?.teamAdjacent?.length > 0) {
            return 'teamAdjacent';
          }
          return resolveCellType(regions, preferred);
        }
    ...
    async function attackRegion () {
      ...
      if (lateCounterActive && !isLateCounterWindow(currentProgress)) {
        lateCounterActive = false;
      }
      ...
      const forceStdParticipationShot = isStdParticipationPending();
    ...
    if (location.href.includes('/teambattle?m=rb')) {
      ...
      if (shouldStartLateCounter(regions, currentProgress)) {
        lateCounterActive = true;
        counterMonitorLogged = false;
        suppressCounterNoTargetLog = false;
      }

      if (!forceStdParticipationShot && !forceStdCounter && !lateCounterActive && Math.abs(nextProgress - currentProgress) >= 2) {
        return;
      }
      ...
      let cellType = forceStdParticipationShot
        ? resolveStdParticipationCellType(regions)
        : resolveCellType(regions);
      ...
      while(dialog.open) {
        ...
        if ((autoJoinMode === 'umeR' || (autoJoinMode === 'counter' && !stdCounterActive)) && !isMorningTime()) {
          ...
          lateCounterActive = false;
          stdParticipationShotPending = true;
          counterMonitorLogged = false;
          excludeSet.clear();
          regions = await getRegions();
          cellType = resolveStdParticipationCellType(regions);
          restartAfterLoop = true;
          logMessage(null, '[切替] umeR/応戦 → stdP', '→ 時間終了');
        } else if (autoJoinMode === 'stdP' && isMorningTime()) {
          autoJoinMode = 'umeR';
          stdCounterActive = false;
          lateCounterActive = false;
          stdParticipationShotPending = false;
          counterMonitorLogged = false;
          excludeSet.clear();
          regions = await getRegions();
          cellType = resolveCellType(regions);
          restartAfterLoop = true;
        ...
        const isLateCounter = lateCounterActive;
        const isUme = autoJoinMode === 'umeR' && !isLateCounter;
        const isCounter = autoJoinMode === 'counter' || isLateCounter;
        const isMorning = isUme || isCounter;

        if (isStdParticipationPending()) {
          cellType = resolveStdParticipationCellType(regions, cellType);
        }
          ...
          try {
            ...
            if (messageType === 'capitalAttack') {
              ...
              if (isCounter) {
                ...
                excludeSet.clear();
                regions = await getRegions();
                cellType = resolveCellType(regions, cellType);
                restartAfterLoop = true;
            ...
            if (success) {
              if (!isMorning && autoJoinMode === 'stdP' && stdParticipationShotPending) {
                stdParticipationShotPending = false;
              }
...
//            logMessage(null, '[打止] 終了です。', next);
        ...
        if (!success && regions[cellType].length === 0) {
          if (isCounter) {
            ...
            if (lateCounterActive && !isLateCounterWindow(currentProgress)) {
              lateCounterActive = false;
              nextProgress = (currentProgress < 50) ? 52 : 2;
              isAutoJoinRunning = false;
              logMessage(null, '[監視停止] late counter終了', `→ ${nextProgress}±1%`);
              return;
            }

            if (!lateCounterActive && shouldStopCounterMonitor(currentProgress)) {
              nextProgress = (currentProgress === 47) ? 52 : 2;
              if (stdCounterActive) {
                autoJoinMode = 'stdP';
                stdCounterActive = false;
              }
              isAutoJoinRunning = false;
              logMessage(null, '[監視停止] ' + currentProgress + '%で停止（47/97誤射防止）', `→ ${nextProgress}±1%`);
              return;
            }
    ...
    async function getRegions () {
      try {
        ...
        const excludedTeamColors = getExcludedTeamColorsForAutoJoin();
        ...
        const isEnemyNonCapitalCell = ([r, c]) => {
          const key = `${r}-${c}`;
          if (capitalSet.has(key)) return false;

          const color = String(cellColors[key] || '').replace('#', '').toUpperCase();
          return !!color && color !== ownTeamColor && !excludedTeamColors.has(color);
        };

        const uniqCells = (targetCells) => {
          const seen = new Set();
        ...
        if (isMorning) {
          ...
          regions.lateCounter = shuffle(uniqCells([
            ...regions.teamAdjacent,
            ...regions.nonAdjacent,
            ...regions.capitalAdjacent,
            ...regions.mapEdge,
            ...regions.onceMore
          ].filter(isEnemyNonCapitalCell)));
        } else {
          const nonAdjacentBase = cells.filter(([r, c]) => {
            const key = `${r}-${c}`;
            return !capitalSet.has(key) && !adjacentSet.has(key);
