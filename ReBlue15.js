...
(()=>{
  ...
  function canInteractTile(r, c) {
    const key = `${r}-${c}`;
    if (liveTileState.visibleSet.has(key)) return true;
    if (liveTileState.exploredSet.has(key)) return true;
    return true;
  }
  ...
  async function handleOverlayPointer(evt) {
    ...
    if (shouldDirectChallengeTile(rc)) {
  ...
  function collectAttackableTilesFromState() {
    ...
    for (const d of dirs) {
      ...
      if (tileOwnedByEnemyTeam(r, c)) {
        result.adjacentEnemy.push({ r, c });
      } else if (tileOwnedByMyTeam(r, c) || tileIsEmpty(r, c)) {
        result.adjacentMove.push({ r, c });
      }
    }
  ...
  async function autoJoinCanvasTick() {
    ...
    try {
      ...
      const target = tiles.adjacentEnemy[0] || tiles.adjacentMove[0];

      if (!target) {
        if (log) {
          const p = document.createElement('p');
          p.textContent = '攻撃可能な隣接マスがありません';
          log.prepend(p);
        }
        isAutoJoinRunning = false;
        return;
      ...
      if (log) {
        const p = document.createElement('p');
        p.textContent = `攻撃: (${target.r}, ${target.c})`;
        log.prepend(p);
      }

      await arenaChallenge(target.r, target.c);
    } catch (e) {
      const log = document.querySelector('.auto-join-log');
      if (log) {
        const p = document.createElement('p');
