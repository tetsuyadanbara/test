...
// @version      1.2c改 2026/03/15修正版 + ランク別PROMAXクリア/UR固定
...
(()=>{
  ...
  (()=> {
    ...
    for(const v of ranks){
      ...
      chkbox_.addEventListener('change', () => {
        if (v === '[UR]') {
          chkbox_.checked = true;
        }
        saveInputData();
  ...
  function saveInputData(){
    const checkedRanks = Array.from(document.querySelectorAll('.keep-item:checked')).map(elm => elm.value);
    if (!checkedRanks.includes('[UR]')) {
      checkedRanks.push('[UR]');
    }
    ...
    if(localStorage.hasOwnProperty('chestOpener')){
      ...
      if (Array.isArray(data.ranks)) {
        data.ranks.forEach(rank => {
          const el = document.querySelector('.keep-item[value="'+rank+'"]');
      ...
      if (data.itemFilters) {
        for(const [key,value] of Object.entries(data.itemFilters)){
          const el = document.querySelector('.wishlist[data-rank="'+key+'"]');
      ...
      if (data.minBuffs) {
        for(const [key,value] of Object.entries(data.minBuffs)){
          const el = document.querySelector('.min-buffs[data-rank="'+key+'"]');
          if (el) el.value = value;
        }
      }
      ...
      if (data.maxDebuffs) {
        for(const [key,value] of Object.entries(data.maxDebuffs)){
          const el = document.querySelector('.max-debuffs[data-rank="'+key+'"]');
          if (el) el.value = value;
        }
      }
    }

    const urCheckbox = document.querySelector('.keep-item[value="[UR]"]');
    if (urCheckbox) {
      urCheckbox.checked = true;
      urCheckbox.disabled = true;
    }
  }
})();
