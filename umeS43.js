...
(()=>{
  ...
  async function fetchAreaInfo(refreshAll) {
    if (isFetchingAreaInfo) return;
    isFetchingAreaInfo = true;

    try {
      const refreshedCells = await refreshArenaInfo();
      if (!refreshedCells) return;
      ...
      cells.forEach((elm) => {
        ...
        if (refreshAll || !hasInfo || isRefreshed) {
          tasks.push(fetchSingleArenaInfo(elm));
        }
      });

      await Promise.allSettled(tasks);
    } finally {
      isFetchingAreaInfo = false;
    }
  }
