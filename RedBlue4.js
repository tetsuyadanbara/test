...
(()=>{
  ...
  async function setPresetItems (presetName, options = {}) {
    ...
    const stat = document.querySelector('.equip-preset-stat');
    if (!force && stat.textContent === '装備中.') return currentEquipName || null;
  ...
  async function autoJoin() {
    ...
    function pickNextProgress() {
      if (currentProgress < 10) {
        ...
        return Math.floor(Math.random() * 5) + 40;
      } else if (currentProgress < 44) {
        return Math.floor(Math.random() * 5) + 56;
      } else if (currentProgress < 60) {
        return Math.floor(Math.random() * 5) + 73;
      } else if (currentProgress < 77) {
        return Math.floor(Math.random() * 5) + 89;
      }
    ...
    async function equipChange(region, excludePresetName = null) {
      ...
      try {
        ...
        let candidates;
        if (excludePresetName) {
          candidates = uniqueSourceList.filter(name => name !== excludePresetName);
          if (!candidates.length && uniqueSourceList.includes(excludePresetName)) {
            candidates = [excludePresetName];
          }
        } else {
          candidates = [...uniqueSourceList];
        }
    ...
    async function executeStep(region, options = {}) {
      ...
      if (isEnemyStep) {
        try {
          ...
          if (equipChangeStat === 'noEquip') {
            return { ok: false, stop: false, retry: false, stale: false, message: `[装備なし] ${cellRank}` };
          }
          if (equipChangeStat === 'noAlternative') {
            return { ok: false, stop: false, retry: false, stale: false, message: `[装備候補不足] ${cellRank}` };
          }
        } catch (e) {
          return { ok: false, stop: false, retry: true, stale: false, message: String(e) };
        }
      }
      ...
      if (messageType === 'equipError') {
        if (isEnemyStep) {
          const samePreset = usedPresetName || currentEquipName || null;

          localStorage.removeItem('current_equip');
          currentEquipName = '';

          try {
            const [retryRank, retryStat, retryPresetName] = await equipChange(region, null);
            ...
            if (retryStat === 'noAlternative') {
              return { ok: false, stop: false, retry: false, stale: false, message: `[装備候補不足] ${retryRank}` };
            }
            if (retryStat === 'noEquip') {
              return { ok: false, stop: false, retry: false, stale: false, message: `[装備なし] ${retryRank}` };
            }
          } catch (_) {}

          if (samePreset) {
            try {
              const [retryRank, retryStat, retryPresetName] = await equipChange(region, samePreset);
              if (retryStat === 'success') {
                return {
                  ok: false,
                  stop: false,
                  ...
                  stale: false,
                  waitSeconds: 3,
                  message: `${lastLine} → 代替装備: ${retryPresetName}`
                };
              }
              if (retryStat === 'noAlternative') {
                return { ok: false, stop: false, retry: false, stale: false, message: `[装備候補不足] ${retryRank}` };
        ...
        return { ok: false, stop: false, retry: true, stale: false, waitSeconds: 3, message: `${lastLine} → 装備再試行待ち` };
      }
      if (messageType === 'reset' || messageType === 'nonAdjacent' || messageType === 'teamAdjacent' || messageType === 'capitalAdjacent' || messageType === 'mapEdge') {
    ...
    async function attackRegion() {
      ...
      if (lastAutoJoinBattlePeriod !== null && currentPeriod !== lastAutoJoinBattlePeriod) {
      ...
      if (currentProgress === 0 || currentProgress === 50 || (location.href.includes('/teambattle?m=rb') && (currentProgress === 16 || currentProgress === 33 || currentProgress === 66 || currentProgress === 83))) {
        ...
        if (currentProgress === 100) {
          ...
          if (location.href.includes('/teambattle?m=rb')) {
             if (currentProgress <= 10) {
               totalSec = (10 - currentProgress) * 600 / 16.6;
             } else if (currentProgress <= 27) {
               totalSec = (27 - currentProgress) * 600 / 16.6;
             } else if (currentProgress <= 44) {
               totalSec = (44 - currentProgress) * 600 / 16.6;
             } else if (currentProgress <= 60) {
               totalSec = (60 - currentProgress) * 600 / 16.6;
             } else if (currentProgress <= 77) {
               totalSec = (77 - currentProgress) * 600 / 16.6;
  ...
  function resetAutoJoinEquipState() {
    currentEquipName = '';
    localStorage.removeItem('current_equip');
    const stat = document.querySelector('.equip-preset-stat');
    if (stat) stat.textContent = '次戦移行のため装備状態を初期化';
  }
