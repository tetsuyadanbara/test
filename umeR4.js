...
(()=>{
  ...
  async function autoJoin() {
    ...
    async function attackRegion () {
      ...
      if (!isBattleWindow(currentProgress)) {
        return;
      }
      ...
      if (nextProgress == null || nextProgress !== stopProgress) {
        nextProgress = stopProgress;
      }
 	...
 	return;
      ...
      try {
        ...
        while (dialog.open) {
          ...
          if (!isBattleWindow(currentProgress) || currentProgress >= nextProgress) {
            logMessage(null, '[停止] 監視停止', `→ ${nextProgress}%で停止`);
            return;
          }
	    ...
	    return;
          ...
          if (autoJoinPhase === 'fill') {
            if (regions.nonAdjacent.length > 0) {
              ...
              lastThreatSnapshot = new Set();
              logMessage(null, '[移行] 空きマス埋め完了 → 残存敵マス掃除 / 自動応戦を開始', `→ ${nextProgress}%で停止`);
	    ...
	    if (targets.length === 0) {
 	      if (!noTargetLogged) {
 	        logMessage(null, '[待機] 応戦対象なし', `→ ${nextProgress}%で停止`);
  	        noTargetLogged = true;
  	      }
  	      return;
  	    }
	    ...
	    if (autoJoinPhase === 'defend') {
          ...
          for (let i = 0; i < targets.length && dialog.open; i++) {
            ...
            while (dialog.open) {
              ...
              if (!isBattleWindow(currentProgress) || currentProgress >= nextProgress) {
                logMessage(region, '[停止] 49% / 99% 到達のため停止', `→ ${nextProgress}%で停止`);
                return;
              }
              ...
              if (equipChangeStat === 'noEquip') {
                excludeSet.add(key);
                logMessage(region, `[除外] 装備不足 (${cellRank})`, `→ ${nextProgress}%で停止`);
                break;
              }
              ...
              if (messageType === 'equipError') {
                excludeSet.add(key);
                noTargetLogged = false;
                logMessage(region, `[除外] ${lastLine} (${cellRank})`, `→ ${nextProgress}%で停止`);
                break;
              }
              ...
              if (messageType === 'guardError') {
                excludeSet.add(key);
                noTargetLogged = false;
                logMessage(region, `[除外] ${lastLine}`, `→ ${nextProgress}%で停止`);
                regions = await getRegions();
                break;
              }
              ...
              if (loop >= 255) {
                excludeSet.add(key);
                logMessage(region, `[打止] (${loop}発目) ${lastLine}`, `→ ${nextProgress}%で停止`);
                break;
              }
