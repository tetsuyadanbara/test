...
(()=>{
  ...
  function goToRC(r, c) {
    const base = `https://donguri.5ch.io/teambattle?r=${r}&c=${c}`;
    location.href = base + getModeQS();
  }

  async function autoExploreUnexplored({steps = 1, rows = 16, cols = 16} = {}) {
    const html = await fetchCurrentHtml();
    const fow = parseFowFromHtml(html);
    if (!fow) return false;

    const explored = buildSetsFromFow(fow);
    const frontier = buildFrontier(explored, rows, cols);
    if (!frontier.length) return false;

    const [r,c] = pickRandom(frontier);

    await sleep(700 + Math.floor(Math.random()*600)); // 700-1300ms
    goToRC(r,c);
