...
(()=>{
    ...
    (()=>{
      ...
      (()=>{ // autoJoinSettingsDialog
        ...
        description.innerText =
          ...
          '除外設定が有効になるのは umeR打止後の counter モードのみで、stdP には影響しません。\n' +
  ...
  async function autoJoin() {
    ...
    function shouldStopLateCounterMonitor(progress) {
      return progress >= 50 && progress < 98 || progress >= 100 || progress === 0;
    }
    ...
    if (location.href.includes('/teambattle?m=rb')) {
      ...
      while(dialog.open) {
        ...
        if ((autoJoinMode === 'umeR' || autoJoinMode === 'counter' || autoJoinMode === 'lateCounter') && !isMorningTime()) {
          autoJoinMode = 'stdP';
...
//            logMessage(null, '[打止] 終了です。', next);
        ...
        if (!success && regions[cellType].length === 0) {
          ...
          if (isLateCounter) {
            ...
            if (shouldStopLateCounterMonitor(currentProgress)) {
              ...
              isAutoJoinRunning = false;
              logMessage(null, '[監視停止] ' + currentProgress + '%でLate counter終了', `→ ${nextProgress}±1%`);
              return;
            }
          ...
          if (location.href.includes('/teambattle?m=rb')) {
            ...
            if (currentProgress < 20) {
              nextProgress = Math.floor(Math.random() * 5) + 28;
            } else if (currentProgress < 35) {
              nextProgress = Math.floor(Math.random() * 5) + 43;
            } else if (currentProgress < 50) {
              nextProgress = Math.floor(Math.random() * 5) + 63;
            } else if (currentProgress < 70) {
              nextProgress = Math.floor(Math.random() * 5) + 78;
            } else if (currentProgress < 85) {
              nextProgress = Math.floor(Math.random() * 5) + 93;
            } else {
              nextProgress = Math.floor(Math.random() * 5) + 13;
            }
          }
    ...
    async function getRegions () {
      try {
        ...
        const isMorning = autoJoinMode === 'umeR' || autoJoinMode === 'counter' || autoJoinMode === 'lateCounter';
        const isCounter = autoJoinMode === 'counter';
        const isLateCounter = autoJoinMode === 'lateCounter';
        const isCounterLike = isCounter || isLateCounter;
        ...
        const filterCounterExcludedTargets = (targets) => {
          if (!isCounterLike) return targets;
        ...
        const filterLateCounterTargets = (targets) => {
          if (!isLateCounter) return targets;
          ...
          return targets.filter(([r, c]) => {
            ...
            if (color === teamColorUpper) return false;
