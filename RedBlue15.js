...
(()=>{
  ...
  function collectAttackableTilesFromState() {
    ...
    for (const d of dirs) {
      ...
      if (!canInteractTile(r, c)) {
        result.blocked.push(rc);
        continue;
      }

      if (tileOwnedByEnemyTeam(r, c)) {
        result.adjacentEnemy.push(rc);
        continue;
      }

      if (tileIsEmpty(r, c)) {
        result.adjacentEmpty.push(rc);
        continue;
      }

      if (tileOwnedByMyTeam(r, c)) {
        result.adjacentOwn.push(rc);
        continue;
      }
  ...
  async function autoJoinCanvasTick() {
    ...
    try {
      ...
      if (tiles.adjacentEnemy.length > 0) {
        target = tiles.adjacentEnemy[0];
        targetType = 'enemy';
      } else if (tiles.adjacentEmpty.length > 0) {
        target = tiles.adjacentEmpty[0];
        targetType = 'empty';
      } else if (tiles.adjacentOwn.length > 0) {
        target = tiles.adjacentOwn[0];
        targetType = 'own';
      }
      ...
      if (!target) {
        if (log) {
          ...
          p.textContent = blocked
            ? `攻撃可能な隣接マスがありません blocked=${blocked}`
            : '攻撃可能な隣接マスがありません';
          log.prepend(p);
        }
        isAutoJoinRunning = false;
      ...
      if (log) {
        const p = document.createElement('p');
        p.textContent = `候補(${targetType}): (${target.r}, ${target.c})`;
        log.prepend(p);
      }
      ...
      if (!ok && log) {
        const p = document.createElement('p');
        p.textContent = `失敗: (${target.r}, ${target.c})`;
        log.prepend(p);
      }
