...
(()=>{
  ...
  function refreshSelfAvatarMarker() {
    ...
    marker.style.left = `${cur.c * cell + cell / 2 - 11}px`;
    marker.style.top = `${cur.r * cell + cell / 2 - 11}px`;

    progressBarInfo.dataset.selfAvatar = `自分: (${cur.r}, ${cur.c})`;
    const baseText = progressBarInfo.textContent.replace(/\s+\|\s+自分: \(\d+, \d+\)$/, '');
