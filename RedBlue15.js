...
(()=>{
  ...
  function shouldAutoMoveTileCompat(rc) {
    ...
    if (tileOwnedByMyTeam(rc.r, rc.c)) return true;
    if (tileIsEmpty(rc.r, rc.c)) return true;
    return false;
  }
  ...
  async function handleOverlayPointer(evt) {
    ...
    if (!canInteractTile(rc.r, rc.c)) return;

    if (evt.type === 'touchstart' && evt.cancelable) {
      evt.preventDefault();
    }

    if (shouldAutoMoveTileCompat(rc)) {
  ...
  function bindOverlayInteraction() {
    const overlay = document.getElementById('gridOverlay');
    if (!overlay) return;
    overlay.addEventListener('click', handleOverlayPointer, { passive: true });
    overlay.addEventListener('touchstart', handleOverlayPointer, { passive: false });
  }
