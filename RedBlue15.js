...
(()=>{
  ...
  (()=>{
    ...
    (()=>{
      ...
      autoJoinDialog.addEventListener('cancel', (e) => {
        e.preventDefault();
      });

      autoJoinDialog.addEventListener('close', () => {
        if (autoJoinDialog.dataset.keepOpen === '1') {
          autoJoinDialog.showModal();
        }
      });
      ...
      (()=>{ // autoJoinSettingsDialog
        ...
        closeButton.addEventListener('click', ()=>{
          autoJoinDialog.dataset.keepOpen = '0';
  ...
  const grid = document.querySelector('#gridWrap') || document.querySelector('.grid');
  const gridWrap = document.getElementById('gridWrap');
  const gridBase = document.getElementById('gridBase');
  const gridFog = document.getElementById('gridFog');
  const gridOverlay = document.getElementById('gridOverlay');
  const gridActionMenu = document.getElementById('gridActionMenu');
  ...
  async function refreshArenaInfo() {
    ...
    try {
      ...
      addCustomColor();
      renderAvatarMarkersOnCustomCells();
    ...
    await Promise.all(
      cells.map(async (elm) => {
        ...
        if (refreshAll || !hasInfo || isRefreshed) {
          await fetchSingleArenaInfo(elm);
        }
      })
    );

    readLiveBattleStateFromPage();
    renderAvatarMarkersOnCustomCells();
  }
  ...
  function getGridSizeFromScripts() {
    const scripts = Array.from(document.querySelectorAll('script'))
      .map(s => s.textContent || '')
      .join('\n');
    const m = scripts.match(/const GRID_SIZE\s*=\s*(\d+)\s*;/);
    return m ? Number(m[1]) : 0;
  }

  function getCellColorsFromScripts() {
    const scripts = Array.from(document.querySelectorAll('script'))
      .map(s => s.textContent || '')
      .join('\n');
    const m = scripts.match(/const cellColors\s*=\s*({[\s\S]*?})\s*;/);
    if (!m) return {};
    try {
      const json = m[1].replace(/'/g, '"').replace(/,\s*}/g, '}');
  ...
  function detectSelfAvatar(avatars) {
    ...
    for (const key of directKeys) {
      if (avatars[key] && typeof avatars[key] === 'object') {
        return avatars[key];
      }
    }

    if (Array.isArray(avatars.avatars)) {
      const tagged = avatars.avatars.find(av =>
        av && (
          av.isSelf === true ||
          av.self === true ||
          av.mine === true ||
  ...
  function readLiveBattleStateFromPage() {
    const fow = window.__FOW || null;
    const avatars = window.__AVATARS || null;

    const visibleSet = new Set();
    const exploredSet = new Set();

    if (fow && Array.isArray(fow.visible)) {
      for (const rc of fow.visible) {
        if (rc && rc.length >= 2) visibleSet.add(`${rc[0]}-${rc[1]}`);
      }
    }
    if (fow && Array.isArray(fow.explored)) {
      for (const rc of fow.explored) {
        if (rc && rc.length >= 2) exploredSet.add(`${rc[0]}-${rc[1]}`);
      }
    }

    let myAvatar = liveTileState.myAvatar ? { ...liveTileState.myAvatar } : null;
    let myTeam = liveTileState.myTeam || '';
    const teamAvatarCoords = [];
    const uniq = new Set();

    function pushTeamAvatar(r, c) {
      if (!Number.isFinite(r) || !Number.isFinite(c)) return;
      const key = `${r},${c}`;
      if (uniq.has(key)) return;
      uniq.add(key);
      teamAvatarCoords.push({ r, c });
    }

    const selfAv = detectSelfAvatar(avatars);
    if (selfAv) {
      const r = avatarRowOf(selfAv);
      const c = avatarColOf(selfAv);
      if (Number.isFinite(r) && Number.isFinite(c)) {
        myAvatar = { r, c };
      }
      myTeam = avatarTeamOf(selfAv) || myTeam;
    }

    if (avatars && Array.isArray(avatars.avatars)) {
      for (const av of avatars.avatars) {
        const r = avatarRowOf(av);
        const c = avatarColOf(av);
        const team = avatarTeamOf(av);
        if (!Number.isFinite(r) || !Number.isFinite(c)) continue;

        if (!myTeam || team === myTeam) {
          pushTeamAvatar(r, c);
        }
    ...
    if (!myAvatar && teamAvatarCoords.length === 1) {
      myAvatar = { ...teamAvatarCoords[0] };
    }

    if (myAvatar) {
      pushTeamAvatar(myAvatar.r, myAvatar.c);
    }

    liveTileState.gridSize = getGridSizeFromScripts();
    liveTileState.mode = parseModeValue();
    liveTileState.cellColors = getCellColorsFromScripts();
    liveTileState.visibleSet = visibleSet;
    liveTileState.exploredSet = exploredSet;
    liveTileState.myAvatar = myAvatar;
    liveTileState.myTeam = myTeam;
    liveTileState.teamAvatarCoords = teamAvatarCoords;

    if (progressBarInfo) {
      const baseText = progressBarInfo.textContent.replace(/\s+\|\s+自分: .*$/, '');
      progressBarInfo.textContent = myAvatar
        ? `${baseText} | 自分: (${myAvatar.r}, ${myAvatar.c})`
        : `${baseText} | 自分: (??, ??)`;
    }
  ...
  function getCanvasCellSize() {
    const overlay = document.getElementById('gridOverlay');
    if (!overlay) return 0;
    const gridSize = liveTileState.gridSize || getGridSizeFromScripts();
    if (!gridSize) return 0;
    const rect = overlay.getBoundingClientRect();
    return rect.width / gridSize;
  }

  function hitTestOverlayToRC(evt) {
    const overlay = document.getElementById('gridOverlay');
    ...
    const cell = getCanvasCellSize();
    const size = liveTileState.gridSize || getGridSizeFromScripts();
  ...
  function currentAvatarRC() {
    const av = liveTileState.myAvatar;
    if (!av) return null;
    if (!Number.isFinite(av.r) || !Number.isFinite(av.c)) return null;
    return { r: av.r, c: av.c };
  }

  function renderAvatarMarkersOnCustomCells() {
    if (!grid) return;

    const self = liveTileState.myAvatar
      ? { r: Number(liveTileState.myAvatar.r), c: Number(liveTileState.myAvatar.c) }
      : null;

    const teamSet = new Set(
      (liveTileState.teamAvatarCoords || []).map(v => `${v.r},${v.c}`)
    );

    const cells = grid.querySelectorAll('.cell');
    cells.forEach(cell => {
      const old = cell.querySelector('.aat-avatar-marker');
      if (old) old.remove();
      ...
      if (self && self.r === row && self.c === col) {
        label = '自';
        borderColor = '#ffd700';
        bgColor = 'rgba(0,0,0,0.82)';
      } else if (teamSet.has(key)) {
        label = '味';
        borderColor = '#00e5ff';
      ...
      marker.className = 'aat-avatar-marker';
  ...
  function ensureSelfAvatarMarker() {
    let marker = document.getElementById('aat-self-avatar-marker');
    if (marker) return marker;

    marker = document.createElement('div');
    marker.id = 'aat-self-avatar-marker';
    marker.textContent = '自';
    marker.title = '侵攻先進部の自分アバター';
    marker.style.position = 'absolute';
    ...
    marker.style.boxShadow = '0 0 0 2px rgba(255,255,255,0.35)';

    if (gridWrap) {
      const pos = getComputedStyle(gridWrap).position;
      if (pos === 'static') {
        gridWrap.style.position = 'relative';
      }
      gridWrap.append(marker);
    }

    return marker;
  }

  function refreshSelfAvatarMarker() {
    const marker = ensureSelfAvatarMarker();
    if (!marker || !gridOverlay) return;

    readLiveBattleStateFromPage();
    const cur = currentAvatarRC();
    const cell = getCanvasCellSize();

    if (!cur || !cell) {
      marker.style.display = 'none';
      progressBarInfo.dataset.selfAvatar = '';
      return;
    }

    marker.style.display = '';
    marker.style.left = `${cur.c * cell + cell / 2 - 12}px`;
    marker.style.top = `${cur.r * cell + cell / 2 - 12}px`;

    progressBarInfo.dataset.selfAvatar = `自分: (${cur.r}, ${cur.c})`;

    const baseText = progressBarInfo.textContent
      .replace(/\s+\|\s+自分: \(\d+, \d+\)$/, '')
      .replace(/\s+\|\s+自分: \(\?\?, \?\?\)$/, '');

    progressBarInfo.textContent = `${baseText} | 自分: (${cur.r}, ${cur.c})`;
  }

  function teamCodeFromColor(hex) {
    const h = String(hex || '').replace('#', '').toLowerCase();
    if (h === 'd32f2f') return 'red';
    if (h === '1976d2') return 'blue';
    return '';
  }

  function tileTeamAt(r, c) {
    return teamCodeFromColor(liveTileState.cellColors[`${r}-${c}`]);
  }

  function tileOwnedByMyTeam(r, c) {
    return !!liveTileState.myTeam && tileTeamAt(r, c) === liveTileState.myTeam;
  }

  function tileOwnedByEnemyTeam(r, c) {
    const t = tileTeamAt(r, c);
    return !!t && !!liveTileState.myTeam && t !== liveTileState.myTeam;
  }

  function tileIsEmpty(r, c) {
    return !tileTeamAt(r, c);
  }
  ...
  function fogStateAtCompat(r, c) {
    const key = `${r}-${c}`;
    if (liveTileState.visibleSet.has(key)) return 2;
    if (liveTileState.exploredSet.has(key)) return 1;
    return 0;
  }

  function canInteractTile(r, c) {
    const hasFogInfo =
      liveTileState.visibleSet.size > 0 ||
      liveTileState.exploredSet.size > 0;

    if (!hasFogInfo) return true;
    return fogStateAtCompat(r, c) !== 0;
  }

  function shouldDirectChallengeTile(rc) {
    const cur = currentAvatarRC();
    if (!cur || !rc) return false;
    if (rc.r === cur.r && rc.c === cur.c) return false;
    if (!isOrthAdjacent(cur, rc)) return false;
    if (!canInteractTile(rc.r, rc.c)) return false;
    if (tileOwnedByEnemyTeam(rc.r, rc.c)) return false;
    return tileIsEmpty(rc.r, rc.c) || tileOwnedByMyTeam(rc.r, rc.c);
  }

  async function handleOverlayPointer(evt) {
    readLiveBattleStateFromPage();

    const rc = hitTestOverlayToRC(evt);
    if (!rc) return;
    ...
    if (!canInteractTile(rc.r, rc.c)) {
      arenaResult.innerText = '未踏（現在は選択できないマスです）';
      arenaResult.show();
      return;
    }
    ...
    if (cur && isOrthAdjacent(cur, rc) && tileOwnedByEnemyTeam(rc.r, rc.c)) {
      arenaResult.innerText = '敵に接敵しています。攻撃/占領タブから攻撃してください';
      arenaResult.show();
      fetchArenaTable(rc.r, rc.c);
      return;
    }

    if (cur && shouldDirectChallengeTile(rc)) {
      await arenaChallenge(rc.r, rc.c);
      return;
    }
  ...
  function bindOverlayInteraction() {
    ...
    overlay.addEventListener('click', async (evt) => {
      await handleOverlayPointer(evt);
      refreshSelfAvatarMarker();
    }, true);

    overlay.addEventListener('touchstart', async (evt) => {
      await handleOverlayPointer(evt);
      refreshSelfAvatarMarker();
    }, { passive: false, capture: true });

    window.addEventListener('resize', refreshSelfAvatarMarker);
    window.addEventListener('scroll', refreshSelfAvatarMarker, true);
  }
  ...
  async function autoEquipAndChallenge (row, col, rank) {
    if (shouldSkipAutoEquip) {
      return await arenaChallenge(row, col);
    }
  ...
  async function arenaChallenge (row, col){
    ...
    body.set('col', String(col));

    const modeFlag = parseModeValue();
    if (modeFlag) body.set('m', modeFlag);

    try {
      const response = await fetch('/teamchallenge', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(text || '/teamchallenge res.ng');
      }

      const failPattern = /失敗|できません|不可|行動不能|未踏|対象外|隣接|弾|too fast|クールダウン|参加して/;
      ...
      liveTileState.myAvatar = { r: Number(row), c: Number(col) };

      if (liveTileState.myTeam) {
        const key = `${row},${col}`;
        const set = new Set((liveTileState.teamAvatarCoords || []).map(v => `${v.r},${v.c}`));
        if (!set.has(key)) {
          (liveTileState.teamAvatarCoords ||= []).push({ r: Number(row), c: Number(col) });
        }
      }

      await drawProgressBar();
      await fetchAreaInfo(false);
      renderAvatarMarkersOnCustomCells();

      if (arenaField.open) {
        await fetchArenaTable(row, col);
      }
  ...
  function collectAttackableTilesFromState() {
    readLiveBattleStateFromPage();
    ...
    if (cur) {
      for (const d of dirs) {
        ...
        if (!canInteractTile(r, c)) continue;
        if (tileOwnedByEnemyTeam(r, c)) {
          result.selfAdjacentEnemy.push({ r, c });
        }
      }
    }

    const uniq = new Set();
    const bases = Array.isArray(liveTileState.teamAvatarCoords) ? liveTileState.teamAvatarCoords : [];
    ...
    for (const base of bases) {
      for (const d of dirs) {
        ...
        if (r < 0 || c < 0 || r >= size || c >= size) continue;
        if (cur && r === cur.r && c === cur.c) continue;

        const key = `${r},${c}`;
        if (uniq.has(key)) continue;
        uniq.add(key);

        if (!canInteractTile(r, c)) {
          result.blocked.push({ r, c });
          continue;
        }

        if (tileIsEmpty(r, c)) {
          result.spawnEmpty.push({ r, c });
        }
      }
    }
  ...
  async function autoJoinCanvasTick() {
    if (isAutoJoinRunning) return;
    isAutoJoinRunning = true;

    try {
      const log = document.querySelector('.auto-join-log');
      const tiles = collectAttackableTilesFromState();

      if (tiles.selfAdjacentEnemy.length > 0) {
        const enemy = tiles.selfAdjacentEnemy[0];

        if (log) {
          const p = document.createElement('p');
          p.textContent = `接敵: (${enemy.r}, ${enemy.c}) 攻撃/占領タブから攻撃してください`;
          log.prepend(p);
        }

        fetchArenaTable(enemy.r, enemy.c);
        isAutoJoinRunning = false;
        return;
      }

      const target = tiles.spawnEmpty[0];

      if (!target) {
        if (log) {
          const p = document.createElement('p');
          const blocked = tiles.blocked.map(v => `(${v.r},${v.c})`).join(' ');
          p.textContent = blocked
            ? `配置可能な空白がありません blocked=${blocked}`
            : '配置可能な空白がありません';
          log.prepend(p);
        }
        isAutoJoinRunning = false;
      ...
      if (log) {
        const p = document.createElement('p');
        p.textContent = `配置候補(empty): (${target.r}, ${target.c})`;
        log.prepend(p);
      }

      const ok = await arenaChallenge(target.r, target.c);

      if (!ok && log) {
        const p = document.createElement('p');
        p.textContent = `配置失敗: (${target.r}, ${target.c})`;
        log.prepend(p);
      }
  ...
  function startAutoJoinCanvasMode() {
    ...
    isAutoJoinRunning = false;
    autoJoinCanvasTick();
    autoJoinIntervalId = setInterval(autoJoinCanvasTick, 3000);
  }
  ...
  function startAutoJoin() {
    ...
    if (dialog) {
      dialog.dataset.keepOpen = '1';
    }

    clearInterval(progressBarIntervalId);
    progressBarIntervalId = null;
    startAutoJoinCanvasMode();
  }
