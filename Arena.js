...
(function () {
  ...
  async function submitChallenge(target, settings = loadLocalSettings()) {
    ...
    if (!isMyTurn(state, settings, lastArenaRows)) return false;
  ...
  async function handleChallengePage() {
    ...
    // 手動popupも自動popupも、そのままログを表示したまま残す
  ...
  async function handleArenaPage() {
    ...
    if (attackableTargets.length > 0) {
      if (!isMyTurn(state, settings, rows)) {
        ...
        await refreshPanel(rows, attackableTargets[0], await loadState(), settings);
        return;
      }
