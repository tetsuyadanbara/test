...
(()=>{
  ...
  async function autoJoin() {
    ...
    function shouldStartLateCounter(regions, progress) {
      return autoJoinMode === 'umeR' &&
        isLateCounterWindow(progress) &&
        (regions?.lateCounter?.length || 0) >= 2;
    }
    ...
    function pickCellType(regions, preferred = null) {
      ...
      const order = isStdAttackTime()
        ? ['teamAdjacent', 'nonAdjacent', 'capitalAdjacent', 'mapEdge', 'onceMore']
        : ['nonAdjacent', 'teamAdjacent', 'capitalAdjacent', 'mapEdge', 'onceMore'];
    ...
    async function attackRegion () {
      ...
      if (lateCounterActive && (!isLateCounterWindow(currentProgress) || autoJoinMode !== 'umeR')) {
    ...
    if (location.href.includes('/teambattle?m=rb')) {
      ...
      if (!lateCounterActive && Math.abs(nextProgress - currentProgress) >= 2) {
        return;
      }
      ...
      while(dialog.open) {
        ...
        if ((autoJoinMode === 'umeR' || autoJoinMode === 'counter') && !isMorningTime()) {
          autoJoinMode = 'stdP';
          lateCounterActive = false;
          counterMonitorLogged = false;
          suppressCounterNoTargetLog = false;
        ...
        if (isCounter && regions[cellType].length === 0) {
          const nextCellType = lateCounterActive
            ? (regions?.lateCounter?.length > 0 ? 'lateCounter' : null)
            : pickCellType(regions);
          if (nextCellType) {
      ...
      try {
       ...
       for (const [r, c] of capitalMap) {
        ...
        const excludedTeamColors = new Set([
          ...fixedExcludedColors,
          ...getExcludedTeamColorsForAutoJoin()
        ]);
