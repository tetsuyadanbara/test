...
(()=>{
  ...
  let lastAutoJoinBattleKey = null;
  let autoJoinEquipResetDoneKey = null;
  ...
  function getBattleSegment(progress) {
    if (!Number.isFinite(progress)) return 'unknown';
    if (MODE === 'm=rb') {
      if (progress < 16) return 'rb-1';
      if (progress < 33) return 'rb-2';
      if (progress < 50) return 'rb-3';
      if (progress < 66) return 'rb-4';
      if (progress < 83) return 'rb-5';
      return 'rb-6';
    }
    if (progress < 50) return 'normal-1';
    return 'normal-2';
  }

  function getBattleKey(period = currentPeriod, progress = currentProgress) {
    if (!Number.isFinite(period) || !Number.isFinite(progress)) return null;
    return `${MODE}|${period}|${getBattleSegment(progress)}`;
  }

  function resetCurrentEquipState(reason = '') {
  ...
  function ensureBattleEquipReset() {
    const battleKey = getBattleKey();
    if (!battleKey) return null;
    if (autoJoinEquipResetDoneKey !== battleKey) {
      resetCurrentEquipState(`第${currentPeriod}期 ${currentProgress}%`);
      autoJoinEquipResetDoneKey = battleKey;
    }
    return battleKey;
  }
  ...
  (()=>{
    ...
    (()=>{
      ...
      function updateAutoJoinDialogPosition() {
        ...
        const preferredWidthPx = Math.min(Math.floor(window.innerWidth * 0.76), 1400);

        const heightPx = Math.max(320, window.innerHeight - topPx - bottomMargin);

        let leftPx = sideMargin;
        let availableWidthPx = window.innerWidth - sideMargin * 2;

        if (toolbarRect.left <= window.innerWidth / 2) {
          leftPx = Math.ceil(toolbarRect.right + gapPx);
          availableWidthPx = window.innerWidth - leftPx - sideMargin;
        } else {
          leftPx = sideMargin;
        ...
        if (availableWidthPx < minWidthPx) {
          leftPx = sideMargin;
          availableWidthPx = window.innerWidth - sideMargin * 2;
        }

        const widthPx = Math.max(minWidthPx, Math.min(availableWidthPx, preferredWidthPx));
      ...
      (()=>{
        ...
        mainRow.style.gridTemplateColumns = 'minmax(0, 1fr) 320px';
        ...
        previewWrap.style.minHeight = '260px';
        ...
        previewViewport.style.minHeight = '0';
  ...
  async function setPresetItems (presetName) {
    ...
    if (!equipPresets[presetName] || !Array.isArray(equipPresets[presetName].id)) {
      throw new Error(`preset.ng: ${presetName}`);
    }
  ...
  async function autoJoin() {
    ...
    async function equipChange(region, excludePresetName = null) {
      const [ row, col ] = region;
      const url = `https://donguri.5ch.io/teambattle?r=${row}&c=${col}&` + MODE;
      try {
        const res = await fetch(url);
        ...
        const sourceList = autoEquipItemsAutojoin[rank]?.length > 0
        ...
        const candidates = excludePresetName
          ? sourceList.filter(name => name !== excludePresetName)
          : [...sourceList];

        if (!candidates.length) {
          return [rank, 'noAlternative', excludePresetName];
        }

        const index = Math.floor(Math.random() * candidates.length);
        const presetName = candidates[index];
        ensureBattleEquipReset();
        await setPresetItems(presetName);
    ...
    async function executeStep(region, options = {}) {
      ...
      if (shouldTryEquip) {
        try {
          ...
          if (equipChangeStat === 'noEquip') {
            return { ok: false, stop: false, retry: false, stale: false, waitSeconds: 0, message: `[装備なし] ${cellRank}` };
          }
          if (equipChangeStat === 'noAlternative') {
            return { ok: false, stop: false, retry: false, stale: false, waitSeconds: 0, message: `[装備候補不足] ${cellRank}` };
          }
        } catch (e) {
          return { ok: false, stop: false, retry: true, stale: false, waitSeconds: 3, message: String(e) };
        }
      }
      ...
      if (
        ...
        return { ok: true, stop: false, retry: false, stale: false, waitSeconds: 0, message: '[成功] ' + lastLine };
      }
      if (messageType === 'breaktime') {
        return { ok: false, stop: true, retry: false, stale: false, waitSeconds: 0, message: lastLine };
      }
      if (messageType === 'quit') {
        clearInterval(autoJoinIntervalId);
        return { ok: false, stop: true, retry: false, stale: false, waitSeconds: 0, message: '[停止] ' + lastLine };
      }
      if (messageType === 'toofast') {
        return { ok: false, stop: false, retry: true, stale: false, waitSeconds: 3, message: lastLine };
      }
      if (messageType === 'retry') {
        return { ok: false, stop: false, retry: true, stale: false, waitSeconds: 20, message: lastLine };
      }
      if (messageType === 'equipError') {
        const failedPresetName = usedPresetName || currentEquipName || null;
        resetCurrentEquipState('equipError');
        if (shouldTryEquip) {
          try {
            const [retryRank, retryStat, retryPresetName] = await equipChange(region, failedPresetName);
            if (retryStat === 'success') {
              return {
                ...
                stale: false,
                waitSeconds: 3,
                message: `${lastLine} → 装備変更: ${retryPresetName}`
              };
            }
        ...
        return { ok: false, stop: false, retry: false, stale: false, waitSeconds: 0, message: lastLine };
      }
      if (
        messageType === 'reset' ||
        ...
        return { ok: false, stop: false, retry: false, stale: true, waitSeconds: 1, message: lastLine };
      }
      if (messageType === 'guardError') {
        return { ok: false, stop: false, retry: false, stale: true, waitSeconds: 1, message: lastLine };
      }
      if (lastLine.length > 100) {
        return { ok: false, stop: false, retry: true, stale: false, waitSeconds: 2, message: 'どんぐりシステム' };
      }
    ...
    async function followPath(path, state) {
      ...
      for (let i = 1; i < path.length; i++) {
        ...
        if (result.stop) {
          return { moved: false, stop: true, keepWatching: false, waitSeconds: 0, message: result.message };
        }
        if (result.retry) {
          return {
            moved: false,
        ...
        if (!result.ok && !result.stale) {
          return { moved: false, stop: false, keepWatching: false, waitSeconds: 0, message: result.message };
        }
    ...
    async function planAndAct() {
      let state = await fetchBattleState();
      let pos = currentPos(state);

      if (!state.hasCapital && !pos) {
      ...
      if (!pos && ownCapital) {
        ...
        if (startResult.stop) {
          return { acted: false, stop: true, keepWatching: false, waitSeconds: 0, message: startResult.message };
        }
      ...
      if (!pos) {
        return { acted: false, stop: false, keepWatching: true, waitSeconds: 2, message: '現在地を取得できませんでした。' };
      }
      ...
      return { acted: false, stop: false, keepWatching: false, waitSeconds: 0, message: '到達可能な空白マス・敵マスが見つかりませんでした。' };
    }
    ...
    async function attackRegion() {
      ...
      const battleKey = getBattleKey();
      if (battleKey && lastAutoJoinBattleKey !== battleKey) {
        resetCurrentEquipState(lastAutoJoinBattleKey ? '新バトル開始' : '自動参加開始');
        autoJoinEquipResetDoneKey = battleKey;
        if (lastAutoJoinBattleKey) {
          logMessage(null, '[装備初期化] 新しいバトルを検知したため装備状態をリセットしました。', '');
        }
        lastAutoJoinBattleKey = battleKey;
      }
      ...
      try {
        ...
        if (result.keepWatching) {
          ...
          setTimeout(() => {
            const dialog = document.querySelector('.auto-join');
            if (dialog?.open && !isAutoJoinRunning) {
              attackRegion();
            }
          }, waitSeconds * 1000);
          return;
        ...
        nextProgress = pickNextProgress();
        const next = `→ ${nextProgress}±1%`;
        logMessage(null, result.message, next);
      } catch (e) {
        console.error(e);
  ...
  async function drawProgressBar(){
    try {
      ...
      if (
        (rbMode && (currentProgress === 0 || currentProgress === 17 || currentProgress === 34 || currentProgress === 51 || currentProgress === 67 || currentProgress === 84)) ||
  ...
  function startAutoJoin() {
    ...
    lastAutoJoinBattleKey = null;
    autoJoinEquipResetDoneKey = null;
    resetCurrentEquipState('自動参加開始');
