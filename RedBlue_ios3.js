...
(()=>{
  ...
  const __aatParams = new URLSearchParams(location.search);
  const __isAutoJoinPreviewFrame = __aatParams.get('_aat_preview') === '1';
  if (__isAutoJoinPreviewFrame) {
    return;
  }
  ...
  (()=>{
    ...
    (()=>{
      ...
      autoJoinDialog.style.position = 'fixed';
      autoJoinDialog.style.margin = '0';
      autoJoinDialog.style.zIndex = '10000';
      autoJoinDialog.style.boxSizing = 'border-box';
      autoJoinDialog.classList.add('auto-join');
      document.body.append(autoJoinDialog);

      function updateAutoJoinDialogPosition() {
        const toolbarRect = toolbar.getBoundingClientRect();
        const viewportW = Math.floor(window.visualViewport?.width || window.innerWidth || document.documentElement.clientWidth || 0);
        const viewportH = Math.floor(window.visualViewport?.height || window.innerHeight || document.documentElement.clientHeight || 0);

        const topPx = Math.ceil(toolbarRect.bottom + 8);
        const bottomMargin = 8;
        const gapPx = 8;
        const sideMargin = 8;
        const narrowDialog = isNarrowViewport || viewportW <= 820;

        let leftPx = sideMargin;
        let availableWidthPx = viewportW - sideMargin * 2;
        let widthPx;

        if (narrowDialog) {
          widthPx = Math.max(280, viewportW - sideMargin * 2);
        } else {
          const minWidthPx = 860;

          // プレビュー大きめ＋右ログ固定幅用
          // 目安: 76vw、ただし最大 1400px まで
          const preferredWidthPx = Math.min(Math.floor(viewportW * 0.76), 1400);

          if (toolbarRect.left <= viewportW / 2) {
            leftPx = Math.ceil(toolbarRect.right + gapPx);
            availableWidthPx = viewportW - leftPx - sideMargin;
          } else {
            leftPx = sideMargin;
            availableWidthPx = Math.floor(toolbarRect.left - gapPx - sideMargin);
          }

          if (availableWidthPx < minWidthPx) {
            leftPx = sideMargin;
            availableWidthPx = viewportW - sideMargin * 2;
          }

          widthPx = Math.max(minWidthPx, Math.min(availableWidthPx, preferredWidthPx));
        }

        const heightPx = Math.max(
          narrowDialog ? 360 : 320,
          viewportH - topPx - bottomMargin
        );

        autoJoinDialog.style.left = `${leftPx}px`;
        autoJoinDialog.style.right = 'auto';
        autoJoinDialog.style.transform = 'none';
        autoJoinDialog.style.top = `${topPx}px`;
        autoJoinDialog.style.width = `${widthPx}px`;
        autoJoinDialog.style.maxWidth = `${widthPx}px`;
        autoJoinDialog.style.height = `${heightPx}px`;
        autoJoinDialog.style.maxHeight = `${heightPx}px`;
        autoJoinDialog.style.overflow = 'hidden';
      }

      window.addEventListener('resize', updateAutoJoinDialogPosition);

      autoJoinButton.addEventListener('click',()=>{
        updateAutoJoinDialogPosition();
      ...
      //autoJoin
      (()=>{
        const autoJoinNarrowLayout = isNarrowViewport;

        const container = document.createElement('div');
        container.style.display = 'grid';
        container.style.gridTemplateRows = 'minmax(0, 1fr) auto auto';
        ...
        mainRow.style.gridTemplateColumns = autoJoinNarrowLayout ? 'minmax(0, 1fr)' : 'minmax(0, 1fr) 320px';
        if (autoJoinNarrowLayout) {
          mainRow.style.gridTemplateRows = 'minmax(0, 1.15fr) minmax(0, 0.85fr)';
        }
        ...
        previewWrap.style.border = 'solid 1px #000';
        previewWrap.style.background = '#fff';
        previewWrap.style.minHeight = autoJoinNarrowLayout ? '0' : '260px';
        ...
        previewHeader.style.padding = '4px 6px';
        previewHeader.style.borderBottom = 'solid 1px #000';
        previewHeader.style.background = '#f5f5f5';
        previewHeader.style.fontSize = '12px';
        previewHeader.style.flex = '0 0 auto';

        const previewTitle = document.createElement('span');
        previewTitle.textContent = 'オリジナル画面';

        const previewStatus = document.createElement('span');
        previewStatus.classList.add('auto-join-preview-status');
        previewStatus.textContent = '待機中';

        const previewViewport = document.createElement('div');
        previewViewport.classList.add('auto-join-preview-viewport');
        previewViewport.style.position = 'relative';
        previewViewport.style.flex = '1 1 auto';
        previewViewport.style.minWidth = '0';
        previewViewport.style.minHeight = autoJoinNarrowLayout ? '140px' : '0';
        previewViewport.style.background = '#fff';
        previewViewport.style.overflow = 'hidden';

        function makePreviewFrame(className, initialOpacity) {
          const frame = document.createElement('iframe');
          frame.classList.add('auto-join-preview-frame', className);
          ...
          frame.style.opacity = initialOpacity;
          frame.style.transition = 'opacity 0.12s linear';
        ...
        const previewFrameA = makePreviewFrame('auto-join-preview-frame-a', '1');
        const previewFrameB = makePreviewFrame('auto-join-preview-frame-b', '0');

        previewHeader.append(previewTitle, previewStatus);
        previewViewport.append(previewFrameA, previewFrameB);
        previewWrap.append(previewHeader, previewViewport);

        const log = document.createElement('div');
        log.style.border = 'solid 1px #000';
        log.style.overflow = 'auto';
        log.style.textAlign = 'left';
        log.style.minHeight = autoJoinNarrowLayout ? '0' : '260px';
        log.style.minWidth = '0';
        ...
        refreshPreviewButton.textContent = '画面更新';
        refreshPreviewButton.addEventListener('click', ()=>{
          if (typeof window.__refreshAutoJoinPreviewNow === 'function') {
            window.__refreshAutoJoinPreviewNow(true);
          }
        })
        ...
        buttonRow.append(settingsButton, refreshPreviewButton, closeButton);

        const p = document.createElement('p');
        p.textContent = 'この画面を開いたままにしておくこと';
        p.style.margin = '0';

        mainRow.append(previewWrap, log);
        container.append(mainRow, p, buttonRow);
  ...
  const battleScrollArea = grid ? grid.parentNode : null;
  let battleZoomLayer = null;
  let manualBattleScale = 1;
  let battlePinchActive = false;
  let battlePinchStartDistance = 0;
  let battlePinchStartScale = 1;

  if (grid && battleScrollArea) {
    battleZoomLayer = document.createElement('div');
    battleZoomLayer.classList.add('battle-zoom-layer');
    battleZoomLayer.style.display = 'block';
    battleZoomLayer.style.width = 'max-content';
    battleZoomLayer.style.maxWidth = 'none';
    battleZoomLayer.style.transformOrigin = 'top left';
    battleZoomLayer.style.willChange = 'transform';

    battleScrollArea.insertBefore(battleZoomLayer, grid);
    battleZoomLayer.append(grid);

    battleScrollArea.style.height = null;
    battleScrollArea.style.maxWidth = '100%';
    battleScrollArea.style.overflowX = 'auto';
    battleScrollArea.style.overflowY = 'hidden';
    battleScrollArea.style.webkitOverflowScrolling = 'touch';

    const canvases = grid.querySelectorAll('canvas');
    canvases.forEach(c => { c.style.display = 'none'; });

    grid.style.display = 'grid';
    grid.style.justifyContent = isNarrowViewport ? 'start' : 'center';
  ...
  function initBattlePinchZoom() {
    if (!isNarrowViewport || !battleScrollArea || !grid) return;
    ...
    const isAutoJoinOpen = () => {
      const dialog = document.querySelector('.auto-join');
      return !!(dialog && dialog.open);
    };

    battleScrollArea.addEventListener('touchstart', (e) => {
      if (isAutoJoinOpen()) return;
      if (!e.touches || e.touches.length < 2) return;

      battlePinchActive = true;
      battlePinchStartDistance = getDistance(e.touches);
      battlePinchStartScale = manualBattleScale;

      e.preventDefault();
    }, { passive: false });

    battleScrollArea.addEventListener('touchmove', (e) => {
      if (isAutoJoinOpen()) return;
      if (!battlePinchActive || !e.touches || e.touches.length < 2) return;

      const distance = getDistance(e.touches);
      if (!distance || !battlePinchStartDistance) return;

      manualBattleScale = Math.max(
        0.35,
        Math.min(1, battlePinchStartScale * (distance / battlePinchStartDistance))
      );

      scaleContentsToFit(battleScrollArea, battleZoomLayer || grid);

      e.preventDefault();
    }, { passive: false });

    const finishPinch = () => {
      battlePinchActive = false;
      battlePinchStartDistance = 0;
      battlePinchStartScale = manualBattleScale;

      if (manualBattleScale > 0.98) {
        manualBattleScale = 1;
      }
    ...
    battleScrollArea.addEventListener('touchend', (e) => {
      if (!e.touches || e.touches.length < 2) {
        finishPinch();
      }
    }, { passive: true });

    battleScrollArea.addEventListener('touchcancel', () => {
      finishPinch();
    }, { passive: true });
  }
  ...
  function scaleContentsToFit(container, contents){
    const target = battleZoomLayer || contents;
    if (!target) return;

    if (isNarrowViewport) {
      const scaleFactor = Math.max(0.35, Math.min(1, manualBattleScale || 1));
      const rawHeight = grid ? grid.scrollHeight : target.scrollHeight;

      target.style.width = 'max-content';
      target.style.maxWidth = 'none';
      target.style.transformOrigin = 'top left';
      target.style.transform = scaleFactor === 1 ? 'none' : `scale(${scaleFactor})`;
      target.style.height = scaleFactor === 1 ? '' : `${Math.ceil(rawHeight * scaleFactor)}px`;
      return;
    }

    const containerWidth = container.clientWidth;
    const contentsWidth = target.scrollWidth;
  ...
  async function refreshArenaInfo() {
    ...
    try {
      ...
      grid.style.gap = '2px';
      grid.style.justifyContent = (isNarrowViewport || settings.mapPosition === 'left') ? 'start' : 'center';
  ...
  async function fetchAreaInfo(refreshAll){
    ...
    if (battleScrollArea) {
      battleScrollArea.style.height = null;
      battleScrollArea.style.padding = '20px 0';
      battleScrollArea.style.maxWidth = '100%';
      battleScrollArea.style.overflowX = 'auto';
      battleScrollArea.style.overflowY = 'hidden';
      battleScrollArea.style.webkitOverflowScrolling = 'touch';
  ...
  async function autoJoin() {
    ...
    function isEnemyFort(state, r, c) {
      const b = state.buildingMap?.[keyOf(r, c)];
      if (!b) return false;
      if (b.kind !== 'f') return false;
      if (state.myTeamCode === 0) return false;
      if (b.team && b.team !== 0) return b.team !== state.myTeamCode;
      return isEnemy(state, r, c);
    }
    ...
    function bfsToEnemyNormalFrontier(state, start) {
      const q = [start];
      const seen = new Set([keyOf(start.r, start.c)]);
      const parent = new Map();
      ...
      while (q.length > 0) {
        ...
        for (const nxt of orthNeighbors(state, cur)) {
          ...
          if (!isEnemy(state, nxt.r, nxt.c)) continue;
          if (isCapitalTile(state, nxt.r, nxt.c)) continue;
          if (isEnemyFort(state, nxt.r, nxt.c)) continue;
    ...
    function bfsToEnemyFortress(state, start) {
      const q = [start];
      const seen = new Set([keyOf(start.r, start.c)]);
      const parent = new Map();
    ...
    function bfsToEnemyFrontier(state, start) {
      return (
        bfsToEnemyNormalFrontier(state, start) ||
        bfsToEnemyFortress(state, start) ||
        bfsToAttackableEnemyCapital(state, start)
      );
    }
    ...
    async function tryClaimOwnCapital(state) {
      ...
      function collectEmptyCapitalCandidates(s) {
        const list = [];

        function isInnerCell(r, c) {
          return r > 0 && c > 0 && r < s.rows - 1 && c < s.cols - 1;
        }

        function edgeDepth(r, c) {
          return Math.min(r, c, s.rows - 1 - r, s.cols - 1 - c);
        }

        for (let r = 0; r < s.rows; r++) {
          for (let c = 0; c < s.cols; c++) {
            if (!isInnerCell(r, c)) continue;   // 四隅・辺縁は除外
            if (!isExplorable(s, r, c)) continue;
            if (isWater(s, r, c)) continue;
            if (!isEmpty(s, r, c)) continue;
            list.push({ r, c });
          }
        }

        list.sort((a, b) => edgeDepth(a.r, a.c) - edgeDepth(b.r, b.c));
        return list;
      }

      const candidates = collectEmptyCapitalCandidates(latestState);
      ...
      for (const cap of candidates) {
        ...
        logMessage(`${cap.r},${cap.c}`, `[首都取得:空白マス] ` + claimResult.message, '');
        ...
        if (claimResult.retry) {
          ...
          if (latestState.hasCapital || getHomeAnchor(latestState)) {
            return {
              acted: true,
              stop: false,
              keepWatching: false,
              waitSeconds: 0,
              message: '首都を取得しました。(空白マス)',
              state: latestState
            };
          }
    ...
    async function planAndAct(stateOverride = null) {
      ...
      if (autoJoinNeedsMidJoinProbe) {
        autoJoinNeedsMidJoinProbe = false;

        if (pos) {
          autoJoinSkipFortifyUntil = Date.now() + 15000;
          lastAutoJoinBattlePeriod = currentPeriod;

          if (location.href.includes('/teambattle?m=rb')) {
            lastAutoJoinRBStage = getCurrentRBStage();
          } else {
            lastAutoJoinRBStage = null;
          }

          logMessage(null, '[途中参戦] 開始直後15秒は要塞建設を抑止します。', '');
        }
      }
      ...
      const shouldSkipOpeningFortify =
        !!pos &&
        Date.now() < autoJoinSkipFortifyUntil;

      if (homeAnchor && !shouldSkipOpeningFortify) {
        ...
        state = fortifyResult.state || state;
        pos = currentPos(state);
      } else if (shouldSkipOpeningFortify) {
        lastAutoFortifyKey = '';
      }
      ...
      const normalEnemyPath = bfsToEnemyNormalFrontier(state, pos);
      if (normalEnemyPath && normalEnemyPath.length >= 2) {
        const result = await followPath(normalEnemyPath, state);
        return {
          acted: result.moved,
          stop: result.stop,
          keepWatching: !!result.keepWatching,
      ...
      const enemyFortPath = bfsToEnemyFortress(state, pos);
      if (enemyFortPath && enemyFortPath.length >= 2) {
        const result = await followPath(enemyFortPath, state);
        return {
          acted: result.moved,
          stop: result.stop,
          keepWatching: !!result.keepWatching,
    ...
    async function runAutoBattleLoop() {
      ...
      for (let loop = 0; loop < 100; loop++) {
        ...
        const nextEnemyPath = bfsToEnemyNormalFrontier(state, pos);
        if (nextEnemyPath && nextEnemyPath.length >= 2) {
          logAutoJoinGuideOnce('enemy', '[継続] 空白が無いため通常の敵マスへ進みます。', null, '');
          await sleep(900);
          if (autoJoinStopRequested) {
            return {
              acted: false,
        ...
        const nextEnemyFortPath = bfsToEnemyFortress(state, pos);
        if (nextEnemyFortPath && nextEnemyFortPath.length >= 2) {
          logAutoJoinGuideOnce('enemyFort', '[継続] 通常の敵が無いため敵要塞を攻めます。', null, '');
          await sleep(900);
          if (autoJoinStopRequested) {
            return {
              acted: false,
        ...
        const nextEnemyCapitalPath = bfsToAttackableEnemyCapital(state, pos);
        if (nextEnemyCapitalPath && nextEnemyCapitalPath.length >= 2) {
          logAutoJoinGuideOnce('enemyCapital', '[継続] 要塞も無いため敵首都を攻めます。', null, '');
          await sleep(900);
          if (autoJoinStopRequested) {
            return {
              acted: false,
    ...
    async function attackRegion() {
      ...
      if (stageChanged) {
        ...
        lastAutoJoinQuietMessage = '';
        lastAutoJoinRBRefreshWaitKey = getAutoJoinRBRefreshWaitKey(currentProgress);
      ...
      if (isAutoJoinRunning || (!stageChanged && Math.abs(nextProgress - currentProgress) >= 2)) {
        return;
      }
  ...
  let lastAutoJoinRBRefreshWaitKey = '';
  let autoJoinSkipFortifyUntil = 0;
  let autoJoinNeedsMidJoinProbe = false;

  function getAutoJoinRBRefreshWaitKey(progress = currentProgress) {
    if (!location.href.includes('/teambattle?m=rb')) return '';
    const p = Number(progress);
    const stage = getRBStage(p);
    if (!Number.isFinite(p) || !stage || !isRBStageRefreshProgress(p)) return '';
    return `${currentPeriod ?? ''}|${stage}|${p}`;
  }
  ...
  let autoJoinPreviewIntervalId = null;
  let autoJoinPreviewLoading = false;
  let autoJoinPreviewSerial = 0;
  let autoJoinPreviewActiveSlot = 'a';

  function setAutoJoinPreviewStatus(text) {
    const status = document.querySelector('.auto-join-preview-status');
    if (status) status.textContent = text;
  }

  function getAutoJoinPreviewFrames() {
    return {
      a: document.querySelector('.auto-join-preview-frame-a'),
      b: document.querySelector('.auto-join-preview-frame-b')
    };
  }

  function buildAutoJoinPreviewUrl() {
    const url = new URL(location.href);
    url.searchParams.set('_aat_preview', '1');
    url.searchParams.set('_aat_preview_ts', String(Date.now()));
    return url.toString();
  }

  function activateAutoJoinPreviewSlot(slot) {
    const frames = getAutoJoinPreviewFrames();
    if (!frames.a || !frames.b) return;

    frames.a.style.opacity = slot === 'a' ? '1' : '0';
    frames.b.style.opacity = slot === 'b' ? '1' : '0';
  }

  function refreshAutoJoinPreview(force = false) {
    const dialog = document.querySelector('.auto-join');
    const frames = getAutoJoinPreviewFrames();

    if (!frames.a || !frames.b) return;
    if (!force && (!dialog || !dialog.open)) return;
    if (autoJoinPreviewLoading && !force) return;

    autoJoinPreviewLoading = true;
    const serial = ++autoJoinPreviewSerial;
    const nextSlot = autoJoinPreviewActiveSlot === 'a' ? 'b' : 'a';
    const targetFrame = frames[nextSlot];

    setAutoJoinPreviewStatus('更新中...');
    ...
    targetFrame.onload = () => {
      if (serial !== autoJoinPreviewSerial) return;
      clearHandlers();
      autoJoinPreviewActiveSlot = nextSlot;
      activateAutoJoinPreviewSlot(nextSlot);
      autoJoinPreviewLoading = false;

      const nowText = new Date().toLocaleTimeString('ja-JP', { hour12: false });
      setAutoJoinPreviewStatus(`更新: ${nowText}`);
    };
    ...
    targetFrame.onerror = () => {
      ...
      clearHandlers();
      autoJoinPreviewLoading = false;
      setAutoJoinPreviewStatus('プレビュー取得失敗');
    };

    targetFrame.src = buildAutoJoinPreviewUrl();
  }

  function stopAutoJoinPreview() {
    if (autoJoinPreviewIntervalId) {
      clearInterval(autoJoinPreviewIntervalId);
      autoJoinPreviewIntervalId = null;
    }

    autoJoinPreviewLoading = false;
    autoJoinPreviewSerial += 1;
    autoJoinPreviewActiveSlot = 'a';

    const frames = getAutoJoinPreviewFrames();
    if (frames.a) {
      frames.a.onload = null;
      frames.a.onerror = null;
      frames.a.src = 'about:blank';
    }
    if (frames.b) {
      frames.b.onload = null;
      frames.b.onerror = null;
      frames.b.src = 'about:blank';
    }

    activateAutoJoinPreviewSlot('a');
    setAutoJoinPreviewStatus('停止中');
  }

  function startAutoJoinPreview() {
    stopAutoJoinPreview();
    refreshAutoJoinPreview(true);
    autoJoinPreviewIntervalId = setInterval(() => {
      if (document.visibilityState === 'hidden') return;
      refreshAutoJoinPreview(false);
    }, 3000);
  }
  ...
  function startAutoJoin() {
    ...
    lastAutoFortifyKey = '';
    lastAutoJoinRBRefreshWaitKey = '';
    lastAutoJoinStatusKey = '';
    lastAutoJoinGuideKey = '';
    lastAutoJoinQuietMessage = '';
    autoJoinSkipFortifyUntil = 0;
    autoJoinNeedsMidJoinProbe = true;

    // 開始直後の1回は必ず「新しい期 / 新しいステージ」として扱わせる
    // ただし途中参戦判定は planAndAct() 側で実施する
    lastAutoJoinBattlePeriod = -1;
...

    {
      ...
      nextProgress = Number.isFinite(p) ? p : 0;
    }

    clearInterval(progressBarIntervalId);
    progressBarIntervalId = null;
    startAutoJoinPreview();
    autoJoin();
  }
  ...
  (()=>{ // autoJoinとprogressBarのinterval管理
    ...
    window.__refreshAutoJoinPreviewNow = refreshAutoJoinPreview;
