...
(()=>{
  ...
  async function autoJoin() {
    ...
    let nextProgress;
    let umeRWindowKey = '';
    let umeRWindowFilledCount = 0;
    let umeRWindowFinished = false;

    function getUmeRWindowInfo() {
      if (MODE === 'm=rb') {
        return { active: false, key: '', nextProgress: nextProgress };
      }
      if (currentProgress >= 48 && currentProgress <= 50) {
        return { active: true, key: `${currentPeriod}-48`, nextProgress: 98 };
      }
      if (currentProgress >= 98 && currentProgress <= 100) {
        return { active: true, key: `${currentPeriod}-98`, nextProgress: 48 };
      }
    ...
    async function attackRegion () {
      ...
      if (useRestrictedUmeR) {
        const windowInfo = getUmeRWindowInfo();
        ...
        if (umeRWindowKey !== windowInfo.key) {
          ...
          umeRWindowFilledCount = 0;
          umeRWindowFinished = false;
        }

        if (umeRWindowFinished) {
          return;
        }
      } else {
        if (Math.abs(nextProgress - currentProgress) >= 2) {
          return;
        }
      }
        ...
        for (let i = 0; i < regions[cellType].length;) {
          ...
          try {
            ...
            if (messageType === 'capitalAttack') {
              ...
              if (useRestrictedUmeR) {
                loop += 1;
                umeRWindowFilledCount += 1;

                if (umeRWindowFilledCount >= 2) {
                  umeRWindowFinished = true;
                  success = true;
                  ...
                  sleepTime = 1;
                  message = '[' + umeRWindowFilledCount + '/2] (' + loop + '発目) ' + lastLine;
                  processType = 'reload';
                }
                i++;
              } else if (isMorning) {
