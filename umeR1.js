...
(()=>{
  ...
  async function autoJoin() {
    ...
    function startCooldown(ms = 60000) {
    ...
    async function attackRegion () {
      ...
      if (isCooldownActive()) {
        ...
        logMessage(null, `[待機] 行動不能クールダウン中`, `→ ${remain}s`);
        return;
      }
        ...
        while (dialog.open) {
          ...
          for (let i = 0; i < targets.length && dialog.open; i++) {
            ...
            while (dialog.open) {
              ...
              if (isCooldownActive()) {
                const remain = Math.ceil((cooldownUntil - Date.now()) / 1000);
                logMessage(region, '[待機] 行動不能クールダウン中', `→ ${remain}s`);
                return;
              }
              ...
              if (
                ...
                startCooldown(60000);
                logMessage(region, '[待機] 行動不能を検知 → 最大60秒待機', '→ 60s');
                return;
              }

              if (messageType === 'toofast') {
                logMessage(region, lastLine, '→ 3s');
                await sleep(3000);
                continue;
              }
