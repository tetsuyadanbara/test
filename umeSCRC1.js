  ...
  async function autoJoin() {
    ...
    function isMorningTime() {
      ...
      // return (hour > 20 || hour < 8) || (hour === 20 && minute >= 30) || (hour === 8 && minute <= 30);
        ...
        let stdInitialShotPending = false;
    ...
    async function attackRegion () {
      ...
      const forceStdInitialShot =
        autoJoinMode === 'stdP' &&
        stdInitialShotPending;

      if (isAutoJoinRunning || (!forceStdCounter && !forceStdInitialShot && Math.abs(nextProgress - currentProgress) >= 2)) {
        return;
      }

      if (forceStdInitialShot) {
        stdInitialShotPending = false;
      }
    ...
    if (location.href.includes('/teambattle?m=rb')) {
      ...
      if (autoJoinMode === 'stdP' && !isMorningTime() && !stdCounterActive && !isAutoJoinRunning) {
        const typeOrder = ['nonAdjacent', 'teamAdjacent', 'capitalAdjacent', 'mapEdge', 'onceMore'];
        const pool = typeOrder.flatMap(type =>
          (regions[type] || []).map(cell => ({ type, cell }))
        );

        if (pool.length > 0) {
          const chosen = pool[Math.floor(Math.random() * pool.length)];
          const chosenKey = chosen.cell.join(',');
          regions[chosen.type] = [
            chosen.cell,
            ...(regions[chosen.type] || []).filter(cell => cell.join(',') !== chosenKey)
          ];
        }
      }
      ...
      while(dialog.open) {
        ...
        if ((autoJoinMode === 'umeR' || (autoJoinMode === 'counter' && !stdCounterActive)) && !isMorningTime()) {
          ...
          if (nextCellType) {
            ...
            stdCounterNoTargetLogged = false;
            ...
            stdCounterActive = false;
            if (!stdCounterNoTargetLogged) {
              stdCounterNoTargetLogged = true;
              logMessage(null, '[待機] 攻撃対象無し', `→ ${currentProgress}%`);
            }
          }
        }
    ...
    async function getRegions () {
      try {
        ...
        if (isMorning) {
          ...
          const nonAdjacentCells = await excludeGuardCells(filteredCells(nonAdjacentBase));
          const capitalAdjacentSafe = await excludeGuardCells(filteredCells(capitalAdjacentCells));
          const teamAdjacentSafe = await excludeGuardCells(filteredCells(teamAdjacentCells));
          const mapEdgeSafe = await excludeGuardCells(filteredCells(mapEdgeCells));
          const onceMoreSafe = await excludeGuardCells(filteredCells(onceMoreBase));
          ...
          if (autoJoinMode === 'counter') {
            const g2 = await excludeGuardCells(group2);
            const g3 = await excludeGuardCells(group3);
            const g4 = await excludeGuardCells(group4);
...
//            nonAdjacent: shuffle(nonAdjacentCells),
   ...
   async function priorityGuardCells(candidates, guardOnly = false) {
    ...
    async function excludeGuardCells(candidates) {
      const checks = candidates.map(async ([r, c]) => {
        const url = `https://donguri.5ch.io/teambattle?r=${r}&c=${c}&` + MODE;
