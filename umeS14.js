...
(()=>{
  ...
  async function fetchSingleArenaInfo(elm) {
    try {
      ...
      function saveCurrentTeamFromCell(cell) {
        if (!cell) return;
        if (!(MODE === 'm=hc' || MODE === 'm=l')) return;

        const teamname = cell.dataset.team || '';
        const customColors = settings.customColors || {};
        const teamcolor = customColors[teamname] || '';

        if (!teamname || !teamcolor) return;

        sessionStorage.setItem('aat_current_team', JSON.stringify({
          teamName: teamname,
          teamColor: teamcolor
        }));
      }
  ...
  async function autoJoin() {
    ...
    const currentTeamSession = JSON.parse(sessionStorage.getItem('aat_current_team') || '{}');

    let teamColor;
    let teamName;

    if (MODE === 'm=hc' || MODE === 'm=l') {
      teamColor = currentTeamSession.teamColor || '';
      teamName = currentTeamSession.teamName || '';
    } else {
      teamColor = currentTeamSession.teamColor || settings.teamColor;
      teamName = currentTeamSession.teamName || settings.teamName;
    }
    ...
    function loadCurrentCapital() {
      try {
        ...
        if (
          ...
          (!saved.teamName || saved.teamName === teamName)
        ) {
    ...
    loadCurrentCapital();
    if ((MODE === 'm=hc' || MODE === 'm=l') && (!teamName || !teamColor)) {
      logArea.innerHTML += '<div>[停止] 先に自軍セルを1回クリックして自軍を確定してください</div>';
      return;
    }
	    ...
	    let ownCapitalAdjacentCells = [];
            ...
            if (MODE === 'm=hc' || MODE === 'm=l') {
              try {
                ...
                ownCapitalAdjacentCells = getCapitalAdjacentCells(saved.row, saved.col, rows, cols)
                  .filter(([r, c]) => !teamColorSet.has(`${r}-${c}`));
              }
            } catch (e) {
              console.error(e);
            }
