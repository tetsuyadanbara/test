    ...
    async function getRegions () {
      try {
        ...
        if (isMorning) {
          ...
          regions.__counterExcludedCount = isCounter ? counterExcludedSkipped : 0;
        } else {
          const nonAdjacentBase = cells.filter(([r, c]) => {
            const key = `${r}-${c}`;
            return !capitalSet.has(key) && !adjacentSet.has(key);
