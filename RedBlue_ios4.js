...
(()=>{
  ...
  if (grid && grid.parentNode) {
    ...
    grid.parentNode.style.webkitOverflowScrolling = 'touch';
  ...
  function initBattlePinchZoom() {
    if (!isNarrowViewport || !grid || !grid.parentNode) return;

    const pinchArea = grid.parentNode;

    const onTouchStart = (e) => {
      if (isAutoJoinDialogOpen()) return;
      if (!e.touches || e.touches.length < 2) return;

      battlePinchActive = true;
      battlePinchStartDistance = getTouchDistance(e.touches);
    ...
    const onTouchMove = (e) => {
      ...
      scaleContentsToFit(pinchArea, grid);
    ...
    pinchArea.addEventListener('touchstart', onTouchStart, { capture: true, passive: false });
    pinchArea.addEventListener('touchmove', onTouchMove, { capture: true, passive: false });
    pinchArea.addEventListener('touchend', onTouchEnd, { capture: true, passive: false });
    pinchArea.addEventListener('touchcancel', onTouchEnd, { capture: true, passive: false });
  ...
  setTimeout(() => {
    try {
      ...
      console.error('initBattlePinchZoom init error:', e);
    }
  }, 0);
  ...
  function applyManualBattleScale(contents) {
    if (!contents) return;

    const prevTransform = contents.style.transform;
    const prevHeight = contents.style.height;

    contents.style.transform = 'none';
    contents.style.height = '';

    const rawWidth = contents.scrollWidth;
    const rawHeight = contents.scrollHeight;

    contents.style.transform = prevTransform;
    contents.style.height = prevHeight;

    const scale = Math.max(0.35, Math.min(1, manualBattleScale || 1));
    contents.style.transform = `scale(${scale})`;
    contents.style.transformOrigin = 'top left';
    contents.style.width = `${rawWidth}px`;
    contents.style.height = `${rawHeight * scale}px`;
  }

  function scaleContentsToFit(container, contents){
    if (isNarrowViewport) {
      applyManualBattleScale(contents);
      return;
    }
