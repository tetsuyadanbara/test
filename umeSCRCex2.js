...
(()=>{
  ...
  async function autoJoin() {
    ...
    if (location.href.includes('/teambattle?m=rb')) {
      ...
      let cellType = forceStdParticipationShot ? 'teamAdjacent' : resolveCellType(regions);
      ...
      while(dialog.open) {
        ...
        if ((autoJoinMode === 'umeR' || (autoJoinMode === 'counter' && !stdCounterActive)) && !isMorningTime()) {
          ...
          cellType = isStdParticipationPending() ? 'teamAdjacent' : resolveCellType(regions);
        ...
        for (let i = 0; i < regions[cellType].length;) {
          ...
          try {
            ...
            if (success) {
              if (!isMorning && autoJoinMode === 'stdP' && cellType === 'teamAdjacent') {
                stdParticipationShotPending = false;
              }
...
//            logMessage(null, '[打止] 終了です。', next);
        ...
        if (!success && regions[cellType].length === 0) {
          ...
          if (!isMorning && autoJoinMode === 'stdP' && stdParticipationShotPending) {
            isAutoJoinRunning = false;
            logMessage(null, '[待機] stdP参戦用の味方隣接マスなし', '→ 30s後再確認');
            return;
          }
    ...
    async function getRegions () {
      try {
        ...
        const excludedTeamColors = getExcludedTeamColors();
