...
(()=>{
  ...
  async function autoJoin() {
    ...
    function shouldStartStdCounter(progress) {
        ...
        function resolveStdPCellType(regions, preferred = null) {
          if (!isStdAttackTime()) {
            return resolveCellType(regions, preferred);
          }

          const order = ['teamAdjacent', 'nonAdjacent', 'capitalAdjacent', 'mapEdge', 'onceMore'];
          if (preferred === 'teamAdjacent' && regions?.teamAdjacent?.length > 0) {
            return 'teamAdjacent';
          }
          return order.find(type => regions[type] && regions[type].length > 0) || 'mapEdge';
        }
    ...
    if (location.href.includes('/teambattle?m=rb')) {
      ...
      if (!forceStdCounter && !lateCounterActive && Math.abs(nextProgress - currentProgress) >= 2) {
        return;
      }
      ...
      let cellType = resolveStdPCellType(regions);
      ...
      while(dialog.open) {
        ...
        if ((autoJoinMode === 'umeR' || (autoJoinMode === 'counter' && !stdCounterActive)) && !isMorningTime()) {
          ...
          cellType = resolveStdPCellType(regions);
        ...
        if (isStdAttackTime()) {
          cellType = resolveStdPCellType(regions, cellType);
        }
