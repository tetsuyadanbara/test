...
(()=>{
  ...
  (()=>{
    ...
    (()=>{
      ...
      (()=>{
        ...
        previewCanvas.classList.add('auto-join-preview-canvas');
        ...
        const previewSourceFrame = document.createElement('iframe');
        previewSourceFrame.classList.add('auto-join-preview-source-frame');
        previewSourceFrame.style.position = 'fixed';
        previewSourceFrame.style.left = '-10000px';
        previewSourceFrame.style.top = '-10000px';
        previewSourceFrame.style.width = '1px';
        previewSourceFrame.style.height = '1px';
        previewSourceFrame.style.opacity = '0';
        previewSourceFrame.style.pointerEvents = 'none';
        previewSourceFrame.style.border = '0';
        previewSourceFrame.style.visibility = 'hidden';
        previewSourceFrame.tabIndex = -1;
        previewSourceFrame.setAttribute('aria-hidden', 'true');

        previewHeader.append(previewTitle, previewStatus);
        previewWrap.append(previewHeader, previewCanvas, previewSourceFrame);
  ...
  function drawAutoJoinPreviewFromDoc(sourceDoc) {
    const previewCanvas = document.querySelector('.auto-join-preview-canvas');
    if (!previewCanvas || !sourceDoc) return false;

    const base = sourceDoc.getElementById('gridBase');
    const fog = sourceDoc.getElementById('gridFog');
    const overlay = sourceDoc.getElementById('gridOverlay');

    if (!base || !overlay) return false;

    const width = Math.max(
      Number(base.width) || 0,
      Number(fog && fog.width) || 0,
      Number(overlay.width) || 0
    );
    const height = Math.max(
      Number(base.height) || 0,
      Number(fog && fog.height) || 0,
      Number(overlay.height) || 0
    );
  ...
  function refreshAutoJoinPreview(force = false) {
    const dialog = document.querySelector('.auto-join');
    const previewCanvas = document.querySelector('.auto-join-preview-canvas');
    const sourceFrame = document.querySelector('.auto-join-preview-source-frame');

    if (!previewCanvas || !sourceFrame) return;
    ...
    const finish = (message) => {
      ...
      setAutoJoinPreviewStatus(message);
    };

    const tryDraw = (retryCount = 20) => {
      if (serial !== autoJoinPreviewSerial) return;

      let sourceDoc = null;
      try {
        sourceDoc = sourceFrame.contentDocument ||
          (sourceFrame.contentWindow ? sourceFrame.contentWindow.document : null);
      } catch (e) {
        finish('プレビュー取得失敗');
        return;
      }

      if (sourceDoc && drawAutoJoinPreviewFromDoc(sourceDoc)) {
      ...
      if (retryCount > 0) {
        setTimeout(() => tryDraw(retryCount - 1), 150);
        return;
      }
    ...
    const onLoad = () => {
      sourceFrame.removeEventListener('load', onLoad);
  ...
  function startAutoJoin() {
    ...
    // これにより、装備初期化後に待機なしで首都取得 or 首都出撃へ進める
