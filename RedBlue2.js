  ...
  (()=>{
    ...
    (()=>{
      ...
      function updateAutoJoinDialogPosition() {
        ...
        // RedBlue38 の 90vw を「toolbar も見せたまま」再現するため、
        // 90vw から toolbar 幅と隙間を引いた値をログ画面の上限にする
        const rb38EquivalentWidthPx = Math.floor(window.innerWidth * 0.9) - Math.ceil(toolbarRect.width) - gapPx;
        ...
        if (toolbarRect.left <= window.innerWidth / 2) {
          ...
          leftPx = sideMargin;
          availableWidthPx = Math.floor(toolbarRect.left - sideMargin - gapPx);
        }
        ...
        const maxWidthPx = Math.max(minWidthPx, rb38EquivalentWidthPx);
        const widthPx = Math.max(minWidthPx, Math.min(availableWidthPx, maxWidthPx));
