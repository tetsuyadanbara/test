...
(()=>{
  ...
  async function autoJoin() {
    ...
    async function tryClaimOwnCapital(state) {
      ...
      const center = {
        r: Math.floor((Math.max(1, latestState.rows) - 1) / 2),
        c: Math.floor((Math.max(1, latestState.cols) - 1) / 2)
      };

      const explicitCapitalCandidates = (latestState.capitalList || [])
        .map(([r, c]) => ({ r, c, fromCapitalList: true }));

      const arbitraryCandidates = [];
      for (let r = 0; r < latestState.rows; r++) {
        for (let c = 0; c < latestState.cols; c++) {
          if (!isExplorable(latestState, r, c)) continue;
          if (isWater(latestState, r, c)) continue;
          arbitraryCandidates.push({ r, c, fromCapitalList: false });
        }
      }

      const seen = new Set();
      const candidates = [...explicitCapitalCandidates, ...arbitraryCandidates]
        .filter(v => {
          const k = keyOf(v.r, v.c);
        ...
        .sort((a, b) => {
          const score = (v) => {
            ...
            if (isEmpty(latestState, v.r, v.c)) s += 0;
            else if (isFriendly(latestState, v.r, v.c)) s += 10;
            else s += 20;

            if (v.fromCapitalList) s -= 3;

            s += Math.abs(v.r - center.r) + Math.abs(v.c - center.c);
            return s;
          };
      ...
      for (const cap of candidates) {
        ...
        const claimLabel = cap.fromCapitalList ? '首都候補' : '任意セル';

        const claimResult = await executeStep([cap.r, cap.c], {
          isEnemyStep: isEnemy(latestState, cap.r, cap.c),
          forceAutoEquip: true
        });
        logMessage(`${cap.r},${cap.c}`, `[首都取得:${claimLabel}] ` + claimResult.message, '');
