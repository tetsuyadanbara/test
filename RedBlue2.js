  ...
  async function autoJoin() {
    ...
    async function executeStep(region, options = {}) {
      ...
      for (let attempt = 0; attempt < 8; attempt++) {
        ...
        if (
          ...
          return { ok: true, stop: false, retry: false, stale: false, message: '[成功] ' + lastLine };
        }
        if (messageType === 'breaktime') {
          return { ok: false, stop: true, retry: false, stale: false, message: lastLine };
        }
        if (messageType === 'quit') {
          clearInterval(autoJoinIntervalId);
          return { ok: false, stop: true, retry: false, stale: false, message: '[停止] ' + lastLine };
        }
        ...
        if (messageType === 'reset' || messageType === 'nonAdjacent' || messageType === 'teamAdjacent' || messageType === 'capitalAdjacent' || messageType === 'mapEdge') {
          return { ok: false, stop: false, retry: false, stale: true, message: lastLine };
        }
      ...
      return { ok: false, stop: false, retry: true, stale: false, message: 'リトライ上限に到達しました。' };
    }

    async function followPath(path, state) {
      if (!Array.isArray(path) || path.length < 2) {
        return { moved: false, stop: false, message: '経路が見つかりませんでした。' };
      }
      ...
      for (let i = 1; i < path.length; i++) {
        ...
        if (result.stop) {
          return { moved: false, stop: true, message: result.message };
        }
        if (!result.ok && !result.stale && !result.retry) {
          return { moved: false, stop: false, message: result.message };
        }
        await sleep(isEnemyStep ? 1500 : 900);
        latestState = await fetchBattleState();
        if (result.stale || result.retry) {
          return { moved: true, stop: false, message: result.message, state: latestState };
        }
      }

      return { moved: true, stop: false, message: '経路完了', state: latestState };
    }
    ...
    async function planAndAct() {
      ...
      if (!pos && ownCapital) {
        ...
        if (startResult.stop) {
          return { acted: false, stop: true, message: startResult.message };
        }
        await sleep(1200);
      ...
      return { acted: false, stop: false, message: '到達可能な空白マス・敵マスが見つかりませんでした。' };
    }
