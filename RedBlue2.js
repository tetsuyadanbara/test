...
(()=>{
  ...
  const EQUIP_LOCK_KEY = 'rb_equip_lock_v1';
  const EQUIP_LOCK_TTL_MS = 15000;
  const EQUIP_LOCK_WAIT_MS = 16000;
  const EQUIP_LOCK_POLL_MS = 250;
  const EQUIP_STABILIZE_MS = 700;
  const equipLockOwner = `${Date.now()}_${Math.random().toString(36).slice(2)}`;

  function readEquipLock() {
    try {
      return JSON.parse(localStorage.getItem(EQUIP_LOCK_KEY) || 'null');
    } catch (e) {
      return null;
    }
  }

  function isEquipLockOwnedByMe(lock = readEquipLock()) {
    return !!(lock && lock.owner === equipLockOwner);
  }

  function isEquipLockStale(lock = readEquipLock()) {
    return !lock || !lock.expiresAt || Date.now() > Number(lock.expiresAt);
  }

  function writeEquipLock() {
    const payload = {
      owner: equipLockOwner,
      expiresAt: Date.now() + EQUIP_LOCK_TTL_MS
    };
    localStorage.setItem(EQUIP_LOCK_KEY, JSON.stringify(payload));
    return payload;
  }

  async function acquireEquipLock(timeoutMs = EQUIP_LOCK_WAIT_MS) {
    const startedAt = Date.now();

    while (Date.now() - startedAt < timeoutMs) {
      const lock = readEquipLock();

      if (!lock || isEquipLockOwnedByMe(lock) || isEquipLockStale(lock)) {
        writeEquipLock();
        await new Promise(resolve => setTimeout(resolve, 80));

        const verifyLock = readEquipLock();
        if (isEquipLockOwnedByMe(verifyLock)) {
          return true;
        }
      }

      await new Promise(resolve => setTimeout(resolve, EQUIP_LOCK_POLL_MS));
    }

    return false;
  }

  function touchEquipLock() {
    if (!isEquipLockOwnedByMe()) return false;
    writeEquipLock();
    return true;
  }

  function releaseEquipLock() {
    const lock = readEquipLock();
    if (isEquipLockOwnedByMe(lock)) {
      localStorage.removeItem(EQUIP_LOCK_KEY);
    }
  }
  ...
  async function setPresetItems (presetName, options = {}) {
    ...
    if (stat) stat.textContent = '装備待機...';

    const locked = await acquireEquipLock();
    if (!locked) {
      if (stat) stat.textContent = '装備ロック待ちタイムアウト';
      throw new Error('装備ロック待ちタイムアウト');
    }
    ...
    try {
      ...
      if (stat) stat.textContent = '装備安定待ち...';
      touchEquipLock();
      await sleep(EQUIP_STABILIZE_MS);
      touchEquipLock();
  ...
  async function autoJoin() {
    ...
    function getPresetEquipIds(presetName) {
      const equipPresets = JSON.parse(localStorage.getItem('equipPresets')) || {};
      const preset = equipPresets[presetName];
      if (!preset || !Array.isArray(preset.id)) return [];
      return normalizeEquipIds(preset.id);
    }

    async function ensurePresetEquippedBeforeAttack(expectedRank, presetName) {
      if (!presetName) return;

      const targetIds = getPresetEquipIds(presetName);
      if (!targetIds.length) return;

      const currentIds = normalizeEquipIds(JSON.parse(localStorage.getItem('current_equip')) || []);
      const samePresetName = currentEquipName === presetName;
      const sameEquipIds = JSON.stringify(targetIds) === JSON.stringify(currentIds);

      if (samePresetName && sameEquipIds) return;

      const stat = document.querySelector('.equip-preset-stat');
      if (stat) stat.textContent = `攻撃前再確認: ${expectedRank} → ${presetName}`;
    ...
    async function equipChange(region, options = {}) {
      ...
      try {
        ...
        const sourceList = [...stageSourceList, ...rankSourceList];
    ...
    async function executeStep(region, options = {}) {
      ...
      if (isEnemyStep) {
        try {
          ...
          await ensurePresetEquippedBeforeAttack(equipRank, usedPresetName);
      ...
      if (messageType === 'equipError') {
        if (isEnemyStep) {
          const preferredPresetName = usedPresetName || currentEquipName || null;
    ...
    async function runAutoBattleLoop() {
      ...
      for (let loop = 0; loop < 100; loop++) {
        ...
        if (nextPriorityEmptyPath && nextPriorityEmptyPath.length >= 2) {
          logMessage(null, '[継続] 首都周囲の空白を優先して埋めます。', '');
          await sleep(300);
          if (autoJoinStopRequested) {
            return {
              acted: false,
        ...
        if (nextEmptyPath && nextEmptyPath.length >= 2) {
          logMessage(null, '[継続] 空白マスを優先して埋めます。', '');
          await sleep(600);
          if (autoJoinStopRequested) {
            return {
              acted: false,
        ...
        if (nextEnemyPath && nextEnemyPath.length >= 2) {
          logMessage(null, '[継続] 空白が無いため敵マスへ進みます。', '');
          await sleep(900);
          if (autoJoinStopRequested) {
            return {
              acted: false,
    ...
    async function attackRegion() {
      ...
      try {
        ...
        if (result.keepWatching) {
          ...
          if (!(result.phase === 'idle' && lastAutoJoinStatusKey === statusKey)) {
