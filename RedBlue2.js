  ...
  (()=>{
    ...
    (()=>{
      ...
      function updateAutoJoinDialogPosition() {
        ...
        // RedBlue38 と同じ「画面の 5vw ～ 95vw」に収まる幅感を基準にする
        const baseLeftPx = Math.floor(window.innerWidth * 0.05);
        const baseRightPx = Math.floor(window.innerWidth * 0.95);

        const heightPx = Math.max(240, window.innerHeight - topPx - bottomMargin);

        let leftPx = baseLeftPx;
        let rightLimitPx = baseRightPx;

        // メニューバーが左側にあるときは、その右端より右から開始
        if (toolbarRect.left <= window.innerWidth / 2) {
          leftPx = Math.max(baseLeftPx, Math.ceil(toolbarRect.right + gapPx));
        } else {
          // メニューバーが右側にあるときは、その左端より左までを使用
          rightLimitPx = Math.min(baseRightPx, Math.floor(toolbarRect.left - gapPx));
        }

        let widthPx = rightLimitPx - leftPx;

        if (widthPx < minWidthPx) {
          leftPx = baseLeftPx;
          rightLimitPx = baseRightPx;
          widthPx = rightLimitPx - leftPx;
        }
