...
// @version      1.3.1-auto
// @description  fix arena ui and add functions (updated specs & add auto join + auto equip)
// @author       7234e634
...
(()=>{
  ...
  window.__AAT__ = window.__AAT__ || {}; // expose minimal APIs for automation
  ...
  // ====== AutoJoin / AutoEquip 追加 ======
  let autoJoinRunning = false;
  let autoJoinStopRequested = false;

  // 自軍セル色(HEX)は設定で持つ。未設定なら色判定なしで動く
  settings.myCellColor = settings.myCellColor || ''; // 例: "#ff0000"
  settings.autoJoinIntervalMs = settings.autoJoinIntervalMs || '1800'; // 次の攻撃まで
  settings.autoJoinBackoffMs  = settings.autoJoinBackoffMs  || '12000'; // too fast / 動き切れ の待機
  settings.autoEquipEnabled   = (settings.autoEquipEnabled ?? true);

  // ランク→プリセット名（あなたの保存済み preset 名に合わせて設定）
  settings.autoEquipPresetByRank = settings.autoEquipPresetByRank || JSON.stringify({
    "N":  "",
    "R":  "",
    "SR": "",
    "SSR": "",
  ...
  function parseRankFromCondText(condText){
    // condText: "SRだけe" / "N-SSR" / "SSR" 等
    if(!condText) return null;
    const order = ["N","R","SR","SSR","UR"];
    const base = condText.includes("-") ? condText.split("-")[1] : condText;
    const m = base.match(/\b(N|R|SR|SSR|UR)\b/);
    if(!m) return null;
    const rank = m[1];
    if(!order.includes(rank)) return null;
    return rank;
  }
  ...
  function pickTargetCellForAutoJoin(){
    const cells = [...document.querySelectorAll('.cell')];
    const selected = [...document.querySelectorAll('.cell.selected')];
    const withInfo = cells.filter(c => c.querySelector('p'));

    const myColor = (settings.myCellColor || '').toLowerCase().trim();

    const base = (selected.length ? selected : (withInfo.length ? withInfo : cells));
    const candidates = base.filter(c=>{
      if(myColor){
        const color = getCellHexColor(c).toLowerCase();
        if(color && color === myColor) return false; // 自軍色なら除外
      }
      return true;
    });
    ...
    const order = ["N","R","SR","SSR","UR"];
    candidates.sort((a,b)=>{
      const ra = parseRankFromCondText(a.querySelector('p')?.textContent || '');
      const rb = parseRankFromCondText(b.querySelector('p')?.textContent || '');
      const ia = ra ? order.indexOf(ra) : 999;
      const ib = rb ? order.indexOf(rb) : 999;
      return ia - ib;
    });
  ...
  async function autoEquipForCell(cell){
    if(!settings.autoEquipEnabled) return true;
    const condText = cell.querySelector('p')?.textContent || '';
    const rank = parseRankFromCondText(condText);
    if(!rank) return true;

    let map;
    try{ map = JSON.parse(settings.autoEquipPresetByRank); }catch{ map = {}; }
    const presetName = (map && map[rank]) ? String(map[rank]).trim() : '';
    if(!presetName) return true;

    // equipPresets に存在するか
    const equipPresets = JSON.parse(localStorage.getItem('equipPresets')) || {};
    if(!equipPresets[presetName]){
      console.warn('[AutoEquip] preset not found:', presetName);
      return true;
    }

    // setPresetItems は装備パネル側IIFEで定義 → window.__AAT__ に公開して呼ぶ
    if(typeof window.__AAT__?.setPresetItems !== 'function'){
      console.warn('[AutoEquip] setPresetItems not exposed yet');
      return true;
    }
    await window.__AAT__.setPresetItems(presetName);
    return true;
  }
  ...
  async function autoJoinLoop(){
    ...
    while(!autoJoinStopRequested){
      try{
        // 情報が無い場合は軽く取得
        if(!shouldSkipAreaInfo){
          const anyInfo = !!document.querySelector('.cell p');
          if(!anyInfo){
        ...
        const cell = pickTargetCellForAutoJoin();
        if(!cell){
        ...
        p.textContent = `[AutoJoin] (${row},${col}) ${lastLine}`;
        ...
        if(
          ...
          await new Promise(r=>setTimeout(r, Number(settings.autoJoinBackoffMs) || 12000));
          continue;
        }
        ...
        if(
          ...
          const p2 = pTemplate.cloneNode();
          p2.textContent = `[AutoJoin] 停止: ${lastLine}`;
          p2.style.fontWeight = 'bold';
          arenaResult.prepend(p2);
          break;
        }

        await new Promise(r=>setTimeout(r, Number(settings.autoJoinIntervalMs) || 1800));
      }catch(e){
        const p = pTemplate.cloneNode();
        p.textContent = `[AutoJoin] 例外: ${e}`;
  ...
  (()=>{
    ...
    autoJoinButton.style.background = '#4b8';
    autoJoinButton.style.color = '#fff';
    function refreshAutoJoinButton(){
      if(autoJoinRunning){
        autoJoinButton.textContent = '■停止';
        autoJoinButton.style.background = '#888';
      }else{
        autoJoinButton.textContent = '▶自動参戦';
        autoJoinButton.style.background = '#4b8';
      }
    }
    refreshAutoJoinButton();
    autoJoinButton.addEventListener('click', ()=>{
      if(autoJoinRunning) stopAutoJoin();
      else startAutoJoin();
      setTimeout(refreshAutoJoinButton, 100);
    });
    ...
    // ★ autoJoinButton を追加
    main.append(menuButton, equipButton, autoJoinButton, sortButton, refreshButton, cellButton);
  ...
  (()=>{
    ...
    (()=>{
      ...
      link.textContent = 'arena assist tool - v1.3.1-auto';
  ...
  (()=>{
    ...
    // ★ AutoEquip から呼べるように公開
    window.__AAT__.setPresetItems = setPresetItems;
