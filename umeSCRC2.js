  ...
  async function autoJoin() {
    ...
    if (location.href.includes('/teambattle?m=rb')) {
      ...
      while(dialog.open) {
        ...
        if (isCounter && regions[cellType].length > 0) {
          counterMonitorLogged = false;
          if (!counterCombatActive && !suppressCounterNoTargetLog) {
            counterCombatActive = true;
          }
        }
        ...
        for (let i = 0; i < regions[cellType].length;) {
          ...
          try {
            ...
            if (messageType === 'capitalAttack') {
              ...
              if (isCounter && text.startsWith('リーダーになった')) {
                counterCombatActive = false;
                suppressCounterNoTargetLog = false;
                counterMonitorLogged = false;
              }
...
//            logMessage(null, '[打止] 終了です。', next);
        ...
        if (!success && regions[cellType].length === 0) {
          if (isCounter) {
            ...
            if (counterCombatActive) {
              counterMonitorLogged = true;
            } else if (suppressCounterNoTargetLog) {
              suppressCounterNoTargetLog = false;
              counterMonitorLogged = false;
            } else if (!counterMonitorLogged) {
              counterMonitorLogged = true;
              logMessage(null, '[監視] counter時間・敵なし', `→ ${counterMonitorMs / 1000}s監視`);
            }
