...
　　const elemOrder = {
  ...
  async function setPresetItems (presetName) {
    ...
    const equipPresets = JSON.parse(localStorage.getItem('equipPresets')) || {};
    if (!equipPresets[presetName] || !Array.isArray(equipPresets[presetName].id)) {
      stat.textContent = '装備プリセットが見つかりません';
      return false;
    }
    ...
    async function getRegions () {
      try {
        ...
        if (isUme) {
          ...
          let lateCounterTargetColors = new Set();
          const lateCounterEnemyCount = isLateCounter
            ? (() => {
                ...
                lateCounterTargetColors = new Set(
                  [...enemyCountByColor.entries()]
                    .filter(([, count]) => count >= 2)
                    .map(([color]) => color)
                );

                return [...enemyCountByColor.entries()]
                  .filter(([, count]) => count >= 2)
          ...
          if (isCounter) {
            ...
            const lateGroup2 = group2.filter(([r, c]) => {
              const key = `${r}-${c}`;
              const color = cellColors[key]?.replace('#', '').toUpperCase();
              return !!color && lateCounterTargetColors.has(color);
            });

            const lateGroup3 = group3.filter(([r, c]) => {
              const key = `${r}-${c}`;
              const color = cellColors[key]?.replace('#', '').toUpperCase();
              return !!color && lateCounterTargetColors.has(color);
            });
